...
class VisualizationDataExporter:
    ...
    def _prepare_events_data(self, events_data: Dict, raw_data: pd.DataFrame) -> List[Dict]:
        ...
        # FIXED: Handle SimpleEventDetector format directly
        if 'events' in events_data and isinstance(events_data['events'], list):
            # Already in correct format from SimpleEventDetector
    ...
    def _prepare_events_summary(self, events_data: Dict) -> Dict:
        ...
        # FIXED: Handle SimpleEventDetector format
        if 'event_summary' in events_data:
            return events_data['event_summary']
    ...
    def _prepare_sentiment_data(self, events_data: Dict) -> Dict:
        ...
        # FIXED: Handle SimpleEventDetector format
        if 'sentiment_analysis' in events_data:
            return events_data['sentiment_analysis']
    ...
    def _prepare_company_info(self, company_info: Dict) -> Dict:
        ...
        # FIXED: Handle both yfinance format and our cached format
        return {
            'name': company_info.get('longName', company_info.get('shortName', '')),
            'sector': company_info.get('sector', ''),
            'industry': company_info.get('industry', ''),
            'country': company_info.get('country', ''),
            'website': company_info.get('website', ''),
            'description': company_info.get('description', company_info.get('longBusinessSummary', '')),
            'employees': company_info.get('employees', company_info.get('fullTimeEmployees', None)),
    ...
    def _prepare_financial_highlights(self, company_info: Dict) -> Dict:
        ...
        return {
            'revenue': company_info.get('revenue', company_info.get('totalRevenue', None)),
            'gross_profit': company_info.get('grossProfit', company_info.get('grossProfits', None)),
