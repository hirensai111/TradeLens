...
def test_enhanced_pipeline():
    ...
    print("[ROCKET] Testing Enhanced Feature Engineering Pipeline...")
    print("=" * 70)
    ...
    try:
        ...
        print(f"[OK] Enhanced feature extraction successful!")
        print(f"   [CHART] Feature Breakdown:")
        ...
        print(f"      [TARGET] Total features: {total_features}")
        print(f"      [TARGET] Target: {features.target:.4f}" if features.target else "      [TARGET] Target: None")
        ...
        for cat_name, cat_features in categories:
            if cat_features:
                ...
                print(f"   [UP] {cat_name}: {sample_items}")
        
        # Enhanced validation
        validation = engineer.validate_enhanced_features(features)
        print(f"\n[OK] Enhanced Feature Validation:")
        ...
        print(f"\n[UP] Testing enhanced training dataset creation...")
        ...
        print(f"[OK] Enhanced training dataset created:")
        print(f"   [CHART] Samples: {len(X)}")
        print(f"   [CHART] Features: {len(X.columns)}")
        print(f"   [CHART] Target range: {y.min():.4f} to {y.max():.4f}")
        print(f"   [CHART] Target std: {y.std():.4f}")
        ...
        try:
            ...
            if importance_analysis:
                ...
                if 'correlation' in importance_analysis:
                    ...
                    print(f"   [CHART] Top 5 features by correlation:")
                    for name, corr in corr_sorted[:5]:
                        print(f"      {name}: {corr:.3f}")
            else:
                print(f"   [WARNING] Feature importance analysis not available (sklearn not installed)")
        except Exception as e:
            print(f"   [WARNING] Feature importance analysis failed: {e}")
        
        # Data quality summary
        print(f"\n[CHART] Data Quality Summary:")
        ...
        print(f"[OK] Successfully created {total_features} features from ALL data sources!")
        print(f"[UP] Ready for advanced ML model training!")
        
    except Exception as e:
        print(f"\n[ERROR] Error testing enhanced pipeline: {e}")
        import traceback
        traceback.print_exc()
