    ...
    def _initialize_phase3_components(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.info("[OK] Database components initialized")
            except ImportError as e:
                self.logger.warning(f"[ERROR] Database components failed: {e}")
                self.session = None
            ...
            try:
                ...
                self.logger.info("[OK] Collector registry initialized (REAL mode)")
                
            except ImportError as e:
                self.logger.warning(f"[ERROR] Collector registry failed: {e}")
                self.collector_registry = None
                self.register_collector = None
            
            # Specific collectors - skip for now due to import issues
            self.logger.info("[OK] Specific collectors initialized with real imports")
            ...
            try:
                ...
                self.logger.info("[OK] Processors initialized")
            except ImportError as e:
                self.logger.warning(f"[ERROR] Processors failed: {e}")
            ...
            try:
                ...
                self.logger.info("[OK] Correlation analyzer initialized")
            except ImportError as e:
                self.logger.warning(f"[ERROR] Correlation analyzer failed: {e}")
            ...
            try:
                ...
                self.logger.info("[OK] Event extractor initialized")
            except ImportError as e:
                self.logger.warning(f"[ERROR] Event extractor failed: {e}")
        ...
        self.logger.info(f"[TARGET] Initialized {available_count}/5 Phase 3 components")
        
        if available_count >= 3:
            self.logger.info("[OK] Phase 3 system is FULLY FUNCTIONAL")
        elif available_count >= 2:
            self.logger.info("[WARNING] Phase 3 system is PARTIALLY FUNCTIONAL")
        else:
            self.logger.warning("[ERROR] Phase 3 system has LIMITED FUNCTIONALITY")
    ...
    def _initialize_search_engine(self):
        ...
        if self.collector_registry and self.register_collector:
            try:
                ...
                self.logger.info("[OK] Search engine components initialized")
            except Exception as e:
                self.logger.error(f"[ERROR] Search engine initialization failed: {e}")
        else:
            self.logger.warning("[WARNING] Search engine initialization skipped - missing collector components")
    ...
    def _setup_collectors(self):
        ...
        if self.HybridNewsCollector and self.register_collector:
            try:
                ...
                collectors_setup += 1
                self.logger.info("[OK] Hybrid News Collector registered")
            except Exception as e:
                self.logger.warning(f"[WARNING] Hybrid collector setup failed: {e}")
        ...
        try:
            ...
            if os.getenv('NEWSAPI_KEY'):
                ...
                collectors_setup += 1
                self.logger.info("[OK] NewsAPI Collector registered")
            else:
                self.logger.info("[INFO] NewsAPI key not found - skipping NewsAPI collector")
        except ImportError:
            self.logger.info("[INFO] NewsAPI collector not available")
        except Exception as e:
            self.logger.warning(f"[WARNING] NewsAPI collector setup failed: {e}")
        ...
        if self.RedditCollector and self.register_collector:
            try:
                ...
                collectors_setup += 1
                self.logger.info("[OK] Reddit Collector registered")
            except Exception as e:
                self.logger.warning(f"[WARNING] Reddit collector setup failed: {e}")
        
        if collectors_setup > 0:
            self.logger.info(f"[TARGET] Search engine setup complete with {collectors_setup} collectors")
            ...
            self.logger.warning("[WARNING] No collectors were successfully setup")
            self.search_engine_available = False
        ...
        try:
            if self.collector_registry:
                ...
                if newsapi_collector and hasattr(newsapi_collector, '_parse_article'):
                    ...
                    self.logger.info("[OK] NewsAPI parsing fix applied")
        except Exception as e:
    ...
    def _collect_fresh_articles(self, ticker: str, limit: int) -> List[Dict]:
        ...
        try:
            if not self.collector_registry:
                self.logger.warning("[WARNING] No collector registry available for fresh collection")
                return []
            ...
            if not available_collectors:
                self.logger.warning("[WARNING] No collectors registered for fresh collection")
                return []
            ...
            for collector_name in available_collectors:
                ...
                if hasattr(collector, 'search_ticker') and collector.search_ticker:
                    self.logger.info(f"   [TARGET] {collector_name} configured for: {collector.search_ticker}")
            ...
            for collector_name, result in collection_results.items():
                if hasattr(result, 'success') and result.success:
                    ...
                    self.logger.info(f"[OK] {collector_name}: {getattr(result, 'articles_processed', 0)} articles")
                else:
                    self.logger.warning(f"[WARNING] {collector_name}: Collection failed")
            ...
            self.logger.error(f"[ERROR] Error collecting fresh articles: {e}")
            return []
        ...
        try:
            ...
            try:
                ...
                if hybrid_collector and hasattr(hybrid_collector, 'set_search_ticker'):
                    ...
                    self.logger.debug(f"[OK] Hybrid collector configured for {ticker}")
            except Exception as e:
            ...
            try:
                ...
                if newsapi_collector and hasattr(newsapi_collector, 'search_queries'):
                    ...
                    self.logger.debug(f"[OK] NewsAPI configured for {ticker}")
            except Exception as e:
            ...
            try:
                ...
                if reddit_collector and hasattr(reddit_collector, 'set_search_ticker'):
                    ...
                    self.logger.debug(f"[OK] Reddit collector configured for {ticker}")
            except Exception as e:
                self.logger.debug(f"Reddit collector configuration failed: {e}")
                
        except Exception as e:
            self.logger.warning(f"[WARNING] Error configuring collectors: {e}")
