...
class APIService {
  ...
  private handleError(error: AxiosError): APIError {
    if (error.response) {
      ...
      // Check if this is a premium access error
      if (error.response.status === 403 && responseData?.error === 'Premium access required') {
        return {
          message: responseData.message || 'Premium access required',
          status: error.response.status,
          code: 'PREMIUM_ACCESS_REQUIRED',
          error: responseData.error,
          available_stocks: responseData.available_stocks || [],
          ticker: responseData.ticker,
        } as PremiumAccessError;
      }
  ...
  // Utility function to check if an error is a premium access error
  static isPremiumAccessError(error: APIError): error is PremiumAccessError {
    return error.code === 'PREMIUM_ACCESS_REQUIRED' && 'available_stocks' in error;
  }

  async getPredictions(ticker: string): Promise<any> {
    try {
      const response = await this.api.get(`/api/predict/${ticker.toUpperCase()}`);
