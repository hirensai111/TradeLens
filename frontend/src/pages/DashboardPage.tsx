...
import { AnalysisResult, PremiumAccessError } from '../types/backend';
import { APIService } from '../services/api';

const DashboardPage: React.FC = () => {
  const { ticker } = useParams<{ ticker: string }>();
  const navigate = useNavigate();
  const { data: analysisData, isLoading, error, analyzeStock, reset } = useStockAnalysis();
  const [searchTicker, setSearchTicker] = useState('');
  useEffect(() => {
    if (ticker) {
      // Call analyzeStock - validation happens in the hook now
      analyzeStock(ticker);
    }
  }, [ticker, analyzeStock]);
  ...
  if (error) {
    // Check if this is a premium access error from the hook
    const isPremiumError = APIService.isPremiumAccessError(error);

    if (isPremiumError) {
      const premiumError = error as import('../types/backend').PremiumAccessError;
      return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
          <div className="max-w-lg w-full bg-white rounded-lg shadow-lg p-8 text-center">
            <div className="text-orange-500 text-6xl mb-4">⚠️</div>
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              {premiumError.ticker?.toUpperCase()} is not available in free tier
            </h2>
            <p className="text-gray-600 mb-6">
              Try one of these popular stocks instead:
            </p>
            <div className="grid grid-cols-3 gap-2 mb-6">
              {premiumError.available_stocks?.map((stock) => (
                <button
                  key={stock}
                  onClick={() => navigate(`/dashboard/${stock}`)}
                  className="bg-blue-50 hover:bg-blue-100 text-blue-700 font-medium py-2 px-3 rounded border border-blue-200 transition-colors"
                >
                  {stock}
                </button>
              ))}
