...
export function useStockAnalysis() {
  ...
  const analyzeStock = useCallback(async (ticker: string, onProgress?: (progress: number, step: string) => void) => {
    // PREMIUM TIER: Stock restrictions removed - all stocks are allowed
    // The backend will handle any necessary validation

    // Proceed with API call for any stock
    try {
      setIsLoading(true);
      setError(null);
      const result = await apiService.analyzeStock(ticker, onProgress);
      setData(result);
      setIsLoading(false);
      return result;
    } catch (error) {
