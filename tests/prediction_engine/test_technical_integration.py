...
print(f"[KEY] Loaded config: API Key: {ZERODHA_CONFIG['api_key'][:8]}...")

# Try to import the analyzer
try:
    from options_analyzer import ZerodhaEnhancedOptionsAnalyzer
    print("[OK] Successfully imported ZerodhaEnhancedOptionsAnalyzer")
except ImportError as e:
    print(f"[ERROR] Failed to import ZerodhaEnhancedOptionsAnalyzer: {e}")
    ...
    try:
        ...
        print("[OK] Technical analyzer imported successfully, let's test that first")
        TEST_TECH_ONLY = True
    except ImportError as e2:
        print(f"[ERROR] Technical analyzer also failed: {e2}")
        sys.exit(1)
else:
...
class MockZerodhaClient:
    ...
    def get_historical_data(self, symbol: str, interval: str, days: int):
        ...
        print(f"[CHART] Mock: Getting {days} days of {interval} data for {symbol}")
...
async def test_technical_only():
    ...
    print("\n[WRENCH] Testing Technical Analyzer Only")
    print("-" * 40)
    ...
    try:
        ...
        for symbol in test_symbols:
            ...
            print(f"[OK] Analysis completed for {symbol}")
        ...
        print(f"[ERROR] Technical analyzer test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
async def test_full_integration():
    ...
    print("\n[ROCKET] Testing Full Integration")
    print("-" * 40)
    ...
    try:
        ...
        symbol = 'RELIANCE'
        print(f"[CHART] Testing full analysis for {symbol}...")
        ...
        print(f"[OK] Full analysis completed for {symbol}")
        
        # Display key results
        if 'technical_analysis' in result:
            tech = result['technical_analysis']
            print(f"[UP] Market Bias: {tech['market_bias']}")
            print(f"[TARGET] Confidence: {tech['confidence_score']:.1%}")
        
        if 'trade_recommendation' in result:
            trade = result['trade_recommendation']
            print(f"[BULB] Strategy: {trade.get('strategy', 'N/A')}")
            print(f"🎖️ Confidence: {trade.get('confidence', 0):.1%}")
            
            if 'entry_rules' in trade:
                entry = trade['entry_rules']
                print(f"[TARGET] Entry: {entry.get('entry_condition', 'N/A')}")
...
async def main():
    """Main test function"""
    print("[ROCKET] Testing Technical Analysis Integration")
    print("=" * 50)
    ...
    if tech_success and not TEST_TECH_ONLY:
        ...
        if full_success:
            ...
            print("\n[WARNING] Technical analyzer works, but full integration has issues")
    elif tech_success:
        print("\n[OK] Technical analyzer test passed!")
        print("[BULB] Next: Fix the options analyzer import to test full integration")
    else:
        print("\n[ERROR] Technical analyzer test failed")
