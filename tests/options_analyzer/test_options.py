...
async def run_comprehensive_test():
    ...
    print("  [OK] Universal bearish bias is FIXED")
    print("  [OK] High confidence signals → Simple directional trades")
    print("  [OK] Technical analysis integration works")
    print("  [OK] Intraday vs swing logic is correct")
    print("  [OK] Strategy creation handles all new strategies")
    print("=" * 80)
    ...
    try:
        ...
        print(f"[WARNING] Using mock analyzer for testing: {e}")
    ...
    for i, scenario in enumerate(TEST_SCENARIOS):
        ...
        try:
            ...
            # Print results
            if result['passed']:
                print(f"[OK] PASSED: {result['summary']}")
            else:
                print(f"[ERROR] FAILED: {result['summary']}")
...
async def run_single_test(analyzer, scenario: Dict) -> Dict:
    ...
    print(f"   [CHART] Symbol: {symbol} | Style: {trading_style}")
    print(f"   [UP] Market: {technical_analysis['market_bias']} bias, {technical_analysis['confidence_score']:.1%} confidence")
    print(f"   [TARGET] Entry Signal: {technical_analysis['entry_signal']['signal_type']} ({technical_analysis['entry_signal']['strength']:.1%})")
    ...
    try:
        ...
        print(f"   [TARGET] Confidence: {strategy['confidence']:.1%}")
...
def generate_test_report(test_results: List[Dict]) -> None:
    ...
    print(f"   [CHART] Success Rate: {(passed_tests/total_tests)*100:.1f}%")
    
    if passed_tests == total_tests:
        print(f"\n🎉 ALL TESTS PASSED! Universal bearish bias is FIXED! 🎉")
        print(f"[OK] High confidence signals now generate directional trades")
        print(f"[OK] Technical analysis integration is working correctly")
        print(f"[OK] Strategy selection logic is properly implemented")
        print(f"[OK] Option legs creation handles all new strategies")
    else:
        print(f"\n[WARNING] SOME TESTS FAILED - NEEDS ATTENTION:")
        
        for result in test_results:
            if not result['passed']:
                print(f"   [ERROR] {result['scenario']}: {result.get('summary', 'Failed')}")
...
class MockAnalyzer:
    ...
    def _generate_zerodha_strategy_with_technical(self, market_data, analyzed_options, technical_analysis, trading_style, risk_tolerance):
        ...
        # **[TARGET] ENHANCED: Check for very strong technical signals first**
...
if __name__ == "__main__":
    print("[ROCKET] Starting Comprehensive Options Analyzer Test Suite...")
