...
try:
    ...
    print("[BULB] python-dotenv not installed. Install with: pip install python-dotenv")
    print("[BULB] Or set environment variables manually")
...
try:
    ...
    try:
        ...
        print(f"[ERROR] Could not import ZerodhaAPIClient: {e}")
...
class ZerodhaAPITester:
    ...
    def setup_client(self):
        ...
        try:
            print("[WRENCH] Setting up Zerodha API client...")
            ...
            if not api_key:
                print("[ERROR] ZERODHA_API_KEY not found in environment variables")
                api_key = input("Enter your Zerodha API Key: ").strip()
            
            if not access_token:
                print("[ERROR] ZERODHA_ACCESS_TOKEN not found in environment variables")
                ...
                try:
                    ...
                    if not request_token or not api_secret:
                        print("[ERROR] Request token and API secret are required")
                        return False
                    ...
                    print(f"[OK] Access Token generated: {access_token}")
                    print("[BULB] Save this access token as ZERODHA_ACCESS_TOKEN environment variable for future use")
                    print(f"[BULB] You can set it by running: set ZERODHA_ACCESS_TOKEN={access_token}")
                    
                except Exception as e:
                    print(f"[ERROR] Error during login flow: {e}")
                    print("[BULB] Common issues:")
                ...
                try:
                    ...
                    print(f"[ERROR] Error initializing client with stored token: {e}")
                    print("[BULB] Your access token might be expired. Try removing it and running the login flow again.")
                    return False
            
            print("[OK] Zerodha API client setup complete")
            return True
            
        except Exception as e:
            print(f"[ERROR] Error setting up client: {e}")
            print("[BULB] Make sure you have the kiteconnect library installed: pip install kiteconnect")
            return False
    
    def test_market_status(self):
        """Test market status functionality"""
        print("\n[CHART] Testing Market Status...")
    ...
    def test_live_quotes(self):
        ...
        try:
            ...
            print(f"[ERROR] Live quotes test failed: {e}")
            self.test_results['live_quotes'] = {
                'status': 'failed',
                'error': str(e)
            }
    ...
    def test_ltp(self):
        ...
        print("\n[TARGET] Testing LTP (Last Traded Price)...")
        ...
        try:
            ...
            print(f"[ERROR] LTP test failed: {e}")
            self.test_results['ltp'] = {
                'status': 'failed',
                'error': str(e)
            }
    ...
    def test_options_data(self):
        ...
        print("\n[UP] Testing Options Data (NIFTY & INFY)...")
        ...
        try:
            ...
            if total_contracts_tested > 0:
                ...
                print("[ERROR] No options contracts found for testing")
                self.test_results['options_data'] = {
                    'status': 'failed',
    ...
    def test_orders(self):
        ...
        try:
            ...
            print(f"[ERROR] Orders test failed: {e}")
            self.test_results['orders'] = {
                'status': 'failed',
                'error': str(e)
            }
    ...
    def test_instruments_loading(self):
        ...
        print("\n[WRENCH] Testing Instruments Loading...")
        ...
        try:
            ...
            if hasattr(self.client, 'symbol_map') and self.client.symbol_map:
                ...
                print("[ERROR] Instruments not loaded")
                self.test_results['instruments'] = {
                    'status': 'failed',
            ...
            print(f"[ERROR] Instruments loading test failed: {e}")
            self.test_results['instruments'] = {
                'status': 'failed',
                'error': str(e)
            }
    ...
    def run_all_tests(self):
        """Run all tests"""
        print("[ROCKET] Starting Zerodha API Comprehensive Tests (NIFTY & INFY Options)")
        print("="*65)
        
        # Setup client
        if not self.setup_client():
            print("[ERROR] Failed to setup client. Exiting...")
            return
        ...
        for test_func in tests:
            try:
                ...
                print(f"[ERROR] Unexpected error in {test_func.__name__}: {e}")
