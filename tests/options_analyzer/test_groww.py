...
try:
    ...
    print("[ERROR] Error: Cannot import GrowwAPIClient. Make sure the groww_api_client.py file is in the same directory.")
    sys.exit(1)
...
class GrowwAPITester:
    ...
    def print_test_result(self, test_name: str, success: bool, message: str = ""):
        ...
        if success:
            ...
            self.failed_tests += 1
            print(f"[ERROR] {test_name}: FAILED {message}")
    ...
    def run_all_tests(self):
        """Run all tests"""
        print(f"\n[ROCKET] Starting Groww API Test Suite - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def print_final_results(self):
        ...
        print(f"\n{'='*60}")
        print(f"[CHART] TEST RESULTS SUMMARY")
        print(f"{'='*60}")
        print(f"[OK] Passed: {self.passed_tests}")
        print(f"[ERROR] Failed: {self.failed_tests}")
        ...
        if success_rate >= 80:
            ...
            print(f"\n[WARNING]  PARTIAL! Some features working. Review your API setup.")
        else:
