#!/usr/bin/env python3
"""
🚀 Automated Indian Intraday Options Bot v2.0 - ENHANCED VERSION
...
class MultiLegStrategyExecutor:
    ...
    async def create_strategy_legs(self, signal: TradingSignal, 
        ...
        if not option_chain:
            logger.error(f"❌ No option chain available for {signal.ticker}")
            return []
        
        logger.info(f"✅ Option chain fetched: {len(option_chain.get('calls', []))} calls, {len(option_chain.get('puts', []))} puts")
        ...
        logger.info(f"✅ Strategy validation passed")
        ...
        if strategy_type == StrategyType.BUY_CALL:
            ...
            logger.info(f"📈 Creating bull call spread")
            ...
            logger.info(f"📉 Creating bear put spread")
        ...
        # Tier 3 only strategies (selling allowed)
            ...
            logger.info(f"💰 Creating credit spread")
            ...
            logger.info(f"📉 Creating short straddle")
            ...
            logger.info(f"📉 Creating short strangle")
        ...
        # Log results
        logger.info(f"🎯 Strategy leg creation completed:")
        ...
        if not legs:
            logger.warning(f"❌ No strategy legs created - check individual strategy method")
    ...
    def _validate_strategy_for_tier(self, strategy_type: StrategyType, capital_tier: CapitalTier) -> bool:
        ...
        if not result_direct and (result_value or result_name):
            logger.warning(f"⚠️  Enum identity issue detected! Using fallback validation.")
    ...
    async def execute_strategy(self, legs: List[OptionsLeg], 
        ...
        for i, leg in enumerate(legs):
            try:
                ...
                if result.get('status') == 'success':
                    ...
                    logger.info(f"✅ Leg {i+1} executed successfully: {result.get('order_id', 'N/A')}")
                else:
                    execution_results['failed_legs'] += 1
                    logger.error(f"❌ Leg {i+1} failed: {result.get('message', 'Unknown error')}")
                ...
                logger.error(f"❌ Exception executing leg {i+1}: {e}")
        ...
        if execution_results['successful_legs'] == len(legs):
            logger.info(f"✅ Strategy fully executed: {execution_results['successful_legs']}/{len(legs)} legs")
        elif execution_results['successful_legs'] > 0:
            logger.warning(f"⚠️ Partial execution: {execution_results['successful_legs']}/{len(legs)} legs")
            execution_results['strategy_executed'] = False
        else:
            logger.error(f"❌ Strategy execution failed: 0/{len(legs)} legs executed")
            execution_results['strategy_executed'] = False
...
class LearningSystem:
    ...
    def _generate_lessons_learned(self, analysis: Dict, patterns: List, lifecycle: sqlite3.Row) -> List[str]:
        ...
        try:
            ...
            if lifecycle['final_pnl'] > 0:
                lessons.append(f"✅ {lifecycle['strategy_type']} profitable in {lifecycle['direction']} direction")
                
                if analysis['max_profit'] > lifecycle['final_pnl'] * 1.5:
                    lessons.append("⚠️ Consider tighter profit-taking rules to avoid giving back gains")
                else:
                    lessons.append("✅ Good profit capture - minimal profit give-back")
            else:
                lessons.append(f"❌ {lifecycle['strategy_type']} unprofitable in {lifecycle['direction']} direction")
                
                if analysis['max_profit'] > 0:
                    lessons.append("💡 Position had profitable moments - improve exit timing")
                else:
                    lessons.append("💡 Review entry criteria - position never profitable")
            ...
            if lifecycle['time_in_position']:
                if lifecycle['time_in_position'] > 120:  # More than 2 hours
                    ...
                    if lifecycle['final_pnl'] > 0:
                        lessons.append("🚀 Quick profitable exit - good momentum recognition")
                    else:
            ...
            if analysis.get('decision_quality', 0) < 0.5:
                lessons.append("🎯 Improve decision-making process - review exit criteria")
            elif analysis.get('decision_quality', 0) > 0.8:
                lessons.append("🎯 Excellent decision-making - maintain current process")
    ...
    def _generate_learning_recommendations(self, insights: Dict) -> List[str]:
        ...
        try:
            ...
            if win_rate > 0.7:
                ...
                recommendations.append("⚠️ Low win rate detected. Review entry criteria and risk management")
            ...
            if strategy_perf:
                ...
                if best_strategies:
                    recommendations.append(
                        f"⭐ Best performing strategies: {', '.join([s[0] for s in best_strategies])}"
                    )
                ...
                if poor_strategies:
                    recommendations.append(
                        f"❌ Avoid these strategies: {', '.join(poor_strategies)}"
                    )
...
class AutomatedIntradayOptionsBot:
    ...
    def __init__(self, zerodha_client: ZerodhaAPIClient):
        ...
        # ✅ FIXED: Initialize core components with correct parameters
        ...
        # ✅ CRITICAL FIX: Pass zerodha_client directly to options analyzer
        ...
        logger.info("✅ AutomatedIntradayOptionsBot initialized with fixed integration")
    ...
    def initialize_v2_components(self):
        ...
        logger.info("✅ v2.0 components initialized successfully")
    ...
    async def enhanced_signal_processing(self, signal: TradingSignal) -> Dict:
        ...
        if not processing_result['approved']:
            logger.info(f"❌ Signal rejected: {processing_result['rejection_reason']}")
            return {'signal_processed': False, 'processing_result': processing_result}
        
        # 2. Get technical analysis with error handling
        try:
            # ✅ FIXED: Use the correct method from your technical analyzer
        ...
        try:
            # ✅ FIXED: Use the options analyzer's analyze_best_options method
        ...
        try:
            ...
            logger.info(f"🎯 Strategy legs created: {len(strategy_legs)} legs")
        ...
        if risk_analysis.get('action') != 'EXIT_IMMEDIATELY':
            ...
            logger.warning(f"⚠️ Risk analysis suggests EXIT_IMMEDIATELY - skipping execution")
        ...
        return {
            ...
            'actual_strategy_legs': strategy_legs,  # ✅ Added actual legs for execution
            'risk_analysis': risk_analysis,
            'execution_results': execution_results,
            'technical_analysis_available': technical_analysis is not None,
            'technical_analysis': technical_analysis,  # ✅ Added for execution method
            'recommended_strategy': recommended_strategy,
            'strategy_type': strategy_type if 'strategy_type' in locals() else StrategyType.BUY_CALL  # ✅ Added strategy type
        }
    
    async def start_automated_trading_v2(self, test_mode: bool = False):
        """Start enhanced automated trading with v2.0 features"""
        
        logger.info("🚀 Starting Enhanced Automated Trading v2.0")
        ...
        try:
            ...
            if not test_mode and not self.bot_config.is_market_hours():
                logger.error("❌ Market is closed - cannot start trading")
                return False
            ...
            logger.info("✅ Enhanced automated trading started successfully")
    ...
    async def _handle_incoming_signal(self, signal: TradingSignal):
        ...
        logger.info(f"📡 Received signal: {signal.ticker} {signal.direction} ({signal.confidence:.1%})")
        ...
        try:
            ...
            logger.error(f"Error handling signal: {e}", exc_info=True)  # ✅ This will show full traceback
    ...
    async def _execute_signal(self, signal: TradingSignal, processing_result: Dict):
        ...
        logger.info(f"🎯 Executing signal: {signal.ticker} {signal.direction}")
        
        try:
            # Debug the processing result first
            logger.info(f"📊 Processing result keys: {list(processing_result.keys())}")
            logger.info(f"📊 Strategy legs count: {processing_result.get('strategy_legs', 0)}")
            ...
            if strategy_legs_count == 0:
                logger.error(f"❌ No strategy legs created for {signal.ticker} - cannot execute")
                self.state = TradingState.IDLE
                return
            ...
            try:
                ...
                logger.info(f"🎯 Created {len(strategy_legs)} strategy legs")
                
                if not strategy_legs:
                    logger.error(f"❌ Strategy leg creation failed for {signal.ticker}")
                    self.state = TradingState.IDLE
                    return
                
            except Exception as e:
                logger.error(f"❌ Error creating strategy legs: {e}")
                self.state = TradingState.IDLE
                return
            ...
            for i, leg in enumerate(strategy_legs):
                try:
                    ...
                    if result.get('status') == 'success':
                        ...
                        logger.info(f"✅ Leg {i+1} executed: {result.get('order_id', 'DRY_RUN')}")
                    else:
                        logger.error(f"❌ Leg {i+1} failed: {result.get('message', 'Unknown error')}")
                        
                except Exception as e:
                    logger.error(f"❌ Error executing leg {i+1}: {e}")
                    execution_results.append({'status': 'error', 'message': str(e)})
            ...
            if successful_orders:
                ...
                self.active_position = ActivePosition(
                    signal=signal,
                    option_legs=strategy_legs,  # ✅ Now we have actual legs!
                ...
                logger.info(f"✅ Position entered: {signal.ticker} with {len(strategy_legs)} legs")
                
            else:
                logger.error(f"❌ All order executions failed for {signal.ticker}")
                self.state = TradingState.IDLE
    ...
    async def manual_analysis(self, ticker: str) -> bool:
        ...
        try:
            ...
            # 5. Log results
            logger.info(f"📊 Manual Analysis Results for {ticker}:")
            ...
            if self.telegram_bot:
                ...
                message += f"💰 Price: ₹{market_data.get('current_price', 0):.2f}\n"
                message += f"📈 Bias: {tech_analysis.get('market_bias', 'NEUTRAL')}\n"
                message += f"🎯 Confidence: {tech_analysis.get('confidence_score', 0):.1%}\n"
    ...
    def _start_signal_interception(self):
        ...
        logger.info("📡 Signal interception started")
    ...
    async def _update_starting_capital(self):
        ...
        try:
            if hasattr(self, 'capital_manager'):
                ...
                logger.info(f"💰 Starting capital updated: ₹{self.daily_performance.starting_capital:,.0f}")
        except Exception as e:
    ...
    async def _send_startup_notification(self, test_mode: bool):
        ...
        try:
            if self.telegram_bot:
                mode = "🧪 TEST MODE" if test_mode else "🚀 LIVE TRADING"
                message = f"{mode}\n\n"
                message += f"💰 Capital: ₹{self.daily_performance.starting_capital:,.0f}\n"
                message += f"📅 Date: {datetime.now().strftime('%Y-%m-%d')}\n"
                message += f"⏰ Started: {datetime.now().strftime('%H:%M:%S')}\n"
                message += f"🎯 Target: 8-15% daily returns"
    ...
    async def _send_entry_notification(self, position: ActivePosition):
        ...
        try:
            if self.telegram_bot:
                message = f"📈 <b>POSITION ENTERED</b>\n\n"
                message += f"📊 <b>{position.signal.ticker}</b> {position.signal.direction.upper()}\n"
                message += f"💰 Entry: ₹{position.entry_price:.2f}\n"
                message += f"🎯 Strategy: {position.strategy_type.value if hasattr(position, 'strategy_type') else 'Unknown'}\n"
        ...
        if position.current_pnl > 0:
            ...
            try:
                ...
                if profit_analysis['action'] == 'TAKE_PROFIT':
                    logger.info(f"💰 Taking profit: {profit_analysis['reason']}")
        ...
        try:
            ...
            for leg in position.option_legs:
                try:
                    ...
                    if result.get('status') == 'success':
                        logger.info(f"✅ Exited {leg.tradingsymbol}: {total_quantity} lots")
                    else:
                        logger.error(f"❌ Exit failed for {leg.tradingsymbol}: {result.get('message')}")
            ...
            logger.info(f"✅ Enhanced exit completed: P&L ₹{final_pnl:+.0f}, Duration: {trade_duration}")
    ...
    def _check_daily_target_status(self) -> Dict:
        ...
        if status['aggressive_target_achieved']:
            ...
            status['status_message'] = "✅ Minimum daily target achieved"
            status['recommendation'] = "Continue trading with conservative approach"
        elif current_roi > 0:
            status['status_message'] = "📈 Positive P&L, working towards target"
            ...
            if loss_percent > 5:
                ...
                status['status_message'] = "📊 Minor daily loss"
    ...
    async def _send_exit_notification(self, position: ActivePosition, exit_reason: ExitReason, 
        ...
        try:
            ...
            message += f"📊 <b>{position.signal.ticker}</b> {position.signal.direction.upper()}\n"
            message += f"💰 P&L: <b>₹{final_pnl:+,.0f}</b> ({roi_percent:+.1f}%)\n"
            ...
            message += f"📈 Daily P&L: ₹{daily_status['current_pnl']:+,.0f} ({daily_status['current_roi_percent']:+.1f}%)\n"
            message += f"🎯 Target: {daily_status['status_message']}"
    ...
    async def run_enhanced_test_suite(self):
        ...
        try:
            ...
            if test_results['overall_success']:
                logger.info("✅ All v2.0 tests passed!")
            else:
                logger.warning("⚠️ Some v2.0 tests failed - check results")
    ...
    async def emergency_shutdown_v2(self):
        ...
        try:
            ...
            if self.active_position:
                ...
                logger.info("✅ Emergency position exit completed")
            ...
            logger.critical("✅ ENHANCED EMERGENCY SHUTDOWN COMPLETED")
    ...
    def _generate_session_recommendations(self) -> List[str]:
        ...
        if self.daily_performance.total_pnl > 0:
            recommendations.append("✅ Profitable session - review successful strategies")
        else:
            recommendations.append("📊 Review losing trades for learning opportunities")
        
        # Win rate recommendations
        win_rate = (self.daily_performance.trades_profitable / max(1, self.daily_performance.trades_executed)) * 100
        if win_rate < 50:
            recommendations.append("🎯 Focus on improving entry criteria and signal quality")
        elif win_rate > 70:
        ...
        if conversion_rate < 30:
            ...
            recommendations.append("⚠️ High signal conversion rate - ensure quality over quantity")
        ...
        if hasattr(self.daily_performance, 'capital_tier'):
            if self.daily_performance.capital_tier == CapitalTier.TIER_1:
                if self.daily_performance.total_pnl > self.daily_performance.starting_capital * 0.05:
                    recommendations.append("💰 Consider upgrading capital for multi-leg strategies")
            elif self.daily_performance.capital_tier == CapitalTier.TIER_2:
                if win_rate > 60:
                    recommendations.append("📈 Good performance - ready for advanced strategies")
            else:  # TIER_3
                if conversion_rate < 40:
                    recommendations.append("🎯 Utilize full strategy arsenal - low conversion rate")
        ...
        if hasattr(self.daily_performance, 'daily_profit_target'):
            ...
            if target_achievement > 100:
                recommendations.append("🎯 Daily target achieved - consider profit protection")
                ...
                recommendations.append("📉 Below target pace - review strategy effectiveness")
    ...
    def setup_signal_interception(self, main_bot_instance, integration_method: str = "direct_callback", **kwargs):
        ...
        try:
            logger.info(f"🔧 Setting up signal interception with method: {integration_method}")
            ...
            if not setup_success:
                logger.error(f"❌ Failed to setup integration method: {integration_method}")
                return False
            ...
            if connection_test['success']:
                logger.info(f"✅ Signal interception setup complete - {integration_method}")
                ...
                logger.warning(f"⚠️ Signal interception setup with warnings: {connection_test['message']}")
                return True  # Still proceed but with warnings
                
        except Exception as e:
            logger.error(f"❌ Error setting up signal interception: {e}", exc_info=True)
            return False
    ...
    def _initialize_signal_components(self):
        ...
        logger.info("🔧 Signal interception components initialized")
    ...
    def _setup_direct_callback(self, **kwargs) -> bool:
        ...
        try:
            ...
            if hasattr(self.main_bot, 'register_signal_callback'):
                ...
                if callback_success:
                    logger.info("✅ Direct callback registered with main bot")
                    self.signal_retrieval_method = 'callback'
                    return True
                else:
                    logger.error("❌ Failed to register callback with main bot")
                    return False
            else:
                logger.error("❌ Main bot does not support callback registration")
                return False
    ...
    def _setup_method_polling(self, method_name: str = "get_latest_signals", 
        ...
        try:
            ...
            if not hasattr(self.main_bot, method_name):
                logger.error(f"❌ Main bot does not have method: {method_name}")
                return False
            ...
            logger.info(f"✅ Method polling setup: {method_name} every {poll_interval}s")
            return True
    ...
    def _setup_shared_queue(self, queue_instance=None, queue_name: str = "signal_queue", **kwargs) -> bool:
        ...
        try:
            if queue_instance:
                ...
                logger.error(f"❌ Shared queue not found: {queue_name}")
                return False
            
            self.signal_retrieval_method = 'shared_queue'
            self.shared_queue_timeout = kwargs.get('timeout', 1.0)
            
            logger.info(f"✅ Shared queue integration setup: {queue_name}")
            return True
    ...
    def _setup_file_watcher(self, signal_file: str = "signals.json", **kwargs) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ File watcher setup: {signal_file}")
            return True
    ...
    def _setup_database_watcher(self, db_path: str = "signals.db", 
        ...
        try:
            ...
            logger.info(f"✅ Database watcher setup: {db_path}/{table_name}")
            return True
    ...
    def _setup_websocket(self, websocket_url: str, **kwargs) -> bool:
        ...
        try:
            ...
            logger.warning("⚠️ WebSocket integration not implemented yet")
            return False
    ...
    def _retrieve_signals(self) -> List[Dict]:
        ...
        try:
            if self.signal_retrieval_method == 'polling':
                ...
                logger.warning(f"⚠️ Unknown signal retrieval method: {self.signal_retrieval_method}")
        ...
        try:
            ...
            if result:
                if isinstance(result, list):
                    ...
                    logger.warning(f"⚠️ Unexpected polling result type: {type(result)}")
    ...
    async def _send_setup_notification(self, integration_method: str, connection_test: Dict):
        ...
        try:
            if self.telegram_bot:
                ...
                message += f"📡 Method: {integration_method}\n"
                message += f"✅ Status: {connection_test['message']}\n"
        ...
        try:
            ...
            if self.signal_thread and self.signal_thread.is_alive():
                ...
                if self.signal_thread.is_alive():
                    logger.warning("⚠️ Signal thread did not stop gracefully")
            
            # Cleanup resources
            self._cleanup_signal_resources()
            
            logger.info("✅ Signal interception stopped")
    ...
    async def receive_signal_from_main_bot(self, signal_data: Dict) -> Dict:
        ...
        try:
            ...
            if not validation_result['valid']:
                ...
                logger.warning(f"❌ Invalid signal data: {validation_result['errors']}")
            ...
            if not queue_result['success']:
                ...
                logger.error(f"❌ Failed to queue signal: {queue_result['error']}")
                return reception_result
            ...
            logger.info(f"📡 Signal received: {signal.ticker} {signal.direction} "
                ...
                if current_size >= max_size:
                    ...
                    try:
                        ...
                        if signal.confidence > 0.8:  # High confidence signal
                            logger.warning(f"⚠️ Queue full, making room for high confidence signal")
    ...
    async def _send_signal_received_notification(self, signal: TradingSignal):
        ...
        try:
            ...
            if signal.confidence >= 0.8 and self.telegram_bot:
                message = f"📡 <b>Signal Received</b>\n\n"
                message += f"📊 {signal.ticker} {signal.direction.upper()}\n"
    ...
    def _generate_performance_recommendations(self, roi_percent: float, win_rate: float, total_trades: int) -> List[str]:
        ...
        if roi_percent >= 15:
            ...
            recommendations.append("📊 Positive returns, room for improvement")
        else:
            recommendations.append("🚨 Negative returns, review strategy and risk management")
        
        # Win rate recommendations
        if win_rate >= 70:
            recommendations.append("🎯 Excellent win rate, consider scaling position sizes")
        elif win_rate >= 50:
            recommendations.append("✅ Good win rate, maintain current approach")
        elif win_rate >= 40:
            recommendations.append("⚠️ Below average win rate, review entry criteria")
        else:
            recommendations.append("🚨 Poor win rate, major strategy review needed")
        
        # Trade frequency recommendations
        if total_trades < 3:
            recommendations.append("📈 Low trade frequency, consider more opportunities")
        elif total_trades > 10:
            recommendations.append("⚠️ High trade frequency, ensure quality over quantity")
...
async def main_v2():
   ...
   print("🚀 Automated Indian Intraday Options Bot v2.0 - ENHANCED")
   print("=" * 70)
       ...
       if len(sys.argv) > 1:
           ...
           if command == 'test':
               ...
               if test_results.get('overall_success'):
                   ...
                   print("⚠️ Some tests failed - check results")
               ...
               print(f"System Health: {'✅ All Components Active' if status.get('system_health', {}).get('all_components_active') else '⚠️ Issues Detected'}")
               ...
               if len(sys.argv) < 3:
                   print("❌ Usage: python bot.py manual <ticker>")
                   return
               ...
               if result:
                   ...
                   print(f"❌ Enhanced analysis failed for {ticker}")
           ...
           print("\n🚀 Production Mode - Starting Enhanced Automated Trading...")
           
           # Test API connection
           market_status = zerodha_client.get_market_status()
           if market_status:
               print(f"✅ Zerodha API connected - Market: {market_status.get('status', 'Unknown')}")
           else:
               print("⚠️ Zerodha API connection issue detected")
           
           print("🔧 Initializing enhanced v2.0 components...")
...
def print_startup_banner():
   ...
   banner = """
╔══════════════════════════════════════════════════════════════╗
║          🚀 AUTOMATED OPTIONS BOT v2.0 ENHANCED 🚀          ║
║                     ⚡ NEXT GENERATION ⚡                    ║
╠══════════════════════════════════════════════════════════════╣
║  🧠 INTELLIGENCE: Dynamic Learning & Pattern Recognition    ║
║  💰 CAPITAL MGMT: 3-Tier Adaptive Strategy Selection        ║
║  🎯 RISK CONTROL: Intelligent Multi-Level Protection        ║
║  📈 OPTIMIZATION: Real-time Profit Enhancement              ║
║  🔄 MONITORING: Continuous Performance Analytics            ║
║  🎨 STRATEGIES: Single → Multi-leg → Advanced Arsenal       ║
╠══════════════════════════════════════════════════════════════╣
║  💎 TIER 1: <₹30K  → Basic Directional Strategies          ║
║  💎 TIER 2: ₹30K-₹1L → Multi-leg Buying Strategies         ║
║  💎 TIER 3: >₹1L → Full Advanced Strategy Arsenal           ║
╠══════════════════════════════════════════════════════════════╣
║  🎯 TARGET: 8-15% Daily Returns with Capital Protection     ║
║  🛡️ SAFETY: Emergency Protocols & Recovery Modes           ║
║  📊 LEARNING: Every Trade Improves Future Performance       ║
╚══════════════════════════════════════════════════════════════╝
   """
