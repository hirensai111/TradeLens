...
from zerodha_api_client import ZerodhaAPIClient  # Changed from GrowwAPIClient
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('indian_trading_bot.log'),
...
class DatabaseSchemaManager:
    ...
    def ensure_automation_schema(self) -> bool:
        """Ensure database has automation-required columns"""
        
        if not os.path.exists(self.db_path):
            # Database doesn't exist yet, will be created properly by IndianTradeLogger
        ...
        try:
            ...
            if migration_needed:
                conn.commit()
                logger.info("✅ Database schema updated for automation integration")
            else:
...
class SmartSignalFilter:
    ...
    def _calculate_signal_quality(self, analysis_result: Dict, current_time: datetime) -> float:
        ...
        logger.info(f"🎯 Quality Score: {final_score:.3f} | {' | '.join(debug_info)}")
        
        # Additional context logging for debugging
        if final_score >= 0.8:
            logger.info(f"🌟 EXCEPTIONAL setup: {final_score:.3f}")
        elif final_score >= 0.7:
            logger.info(f"✅ STRONG setup: {final_score:.3f}")
        elif final_score >= 0.6:
            logger.info(f"👍 GOOD setup: {final_score:.3f}")
        else:
            logger.info(f"⚠️ WEAK setup: {final_score:.3f}")
...
class IndianTradingBot:
    ...
    def __init__(self, config: BotConfig = None):
        ...
        logger.info("🚀 Initializing Indian Trading Bot components with Zerodha Kite Connect API...")
        ...
        try:
            ...
            try:
                ...
                if automation_test.get('status') == 'success':
                    logger.info("✅ Automation integration verified")
                else:
                    logger.warning(f"⚠️ Automation integration issue: {automation_test.get('message')}")
            except Exception as e:
            ...
            logger.info("✅ All components initialized successfully with Zerodha Kite Connect API")
            logger.info("✅ Database automation integration ready")
    ...
    def _load_config(self) -> BotConfig:
        ...
        # With this (exact watchlist from Enhanced Analyzer):
        ...
        return BotConfig(
            ...
            INDIAN_WATCHLIST=[ticker.strip() for ticker in watchlist],
    ...
    def start_bot(self):
        ...
        try:
            logger.info("🇮🇳 Starting Indian Trading Signal Bot with Zerodha Kite Connect API...")
            ...
            self.telegram_bot.send_message(
                "🚀 Indian Trading Bot Started with Zerodha Kite Connect API!\n"
                f"📊 Monitoring: {', '.join(self.config.INDIAN_WATCHLIST[:5])}...\n"
                f"⏰ Scan interval: {self.config.SCAN_INTERVAL_MINUTES} min\n"
                f"💡 Min confidence: {self.config.MIN_CONFIDENCE_SCORE:.0%}\n"
            ...
            for hour, minute in scan_times:
                scheduler.add_job(
                    func=self._async_job_wrapper(self.market_scan_cycle),  # ✅ FIXED: Use async wrapper
                    trigger="cron",
                    hour=hour, 
                    minute=minute,
                    day_of_week='mon-fri',
        ...
        try:
            ...
            if enhanced_gap_analysis['gaps_detected']:
                ...
                for gap_info in enhanced_gap_analysis['gap_setups'][:4]:  # Top 4 gap setups
                    gap_emoji = "📈" if gap_info['gap_type'] == 'GAP_UP' else "📉"
                    strategy_emoji = "🎯" if gap_info['confidence'] > 0.7 else "⚠️"
                ...
                summary_parts.extend([
                    f"📈 Total Gap Ups: {total_gap_ups} | 📉 Gap Downs: {total_gap_downs}",
                    f"🎯 High Confidence Setups: {len([g for g in enhanced_gap_analysis['gap_setups'] if g['confidence'] > 0.7])}",
                    ""
                ])
            else:
                summary_parts.extend([
            ...
            if pre_market_opportunities['high_confidence_setups']:
                ...
                for opp in pre_market_opportunities['high_confidence_setups'][:3]:  # Top 3
                    confidence_stars = "⭐⭐⭐⭐⭐" if opp['confidence'] > 0.8 else "⭐⭐⭐⭐" if opp['confidence'] > 0.7 else "⭐⭐⭐"
            ...
            if market_regime:
                ...
                regime_emoji = "🔥" if volatility_regime == 'HIGH' else "😴" if volatility_regime == 'LOW' else "📊"
            ...
            logger.info("✅ Enhanced pre-market analysis completed")
            logger.info(f"📊 Gaps detected: {len(enhanced_gap_analysis.get('gap_setups', []))}")
            logger.info(f"🎯 High-confidence opportunities: {len(pre_market_opportunities.get('high_confidence_setups', []))}")
    ...
    async def _send_gap_strategy_alerts(self, gap_setups: List[Dict]):
        ...
        for gap in significant_gaps[:3]:  # Top 3 only
            strategy_emoji = "🚀" if 'CONTINUATION' in gap['recommended_strategy'] else "🔄" if 'REVERSAL' in gap['recommended_strategy'] else "📊"
        ...
        alert_parts.append("⚠️ Execute with proper risk management!")
    ...
    async def market_scan_cycle(self):
        ...
        try:
            ...
            logger.info(f"✅ Scan completed: {signals_found} signals found in {scan_duration:.1f}s")
    ...
    def _should_send_signal(self, analysis_result: Dict, ticker: str) -> bool:
        ...
        logger.info(f"✅ Signal APPROVED for {ticker}: {approval_details}")
    ...
    def _process_and_send_signal(self, ticker: str, analysis_result: Dict) -> bool:
        """ENHANCED signal processing with better automation integration"""
        
        try:
            # UPDATED: Extract from new structure
            trade_rec = analysis_result['trade_recommendation']
            confidence = trade_rec.get('confidence', 0)
            ...
            if message_sent:
                ...
                logger.info(f"✅ Signal sent for {ticker}: {trade_rec.get('strategy')} ({confidence:.0%})")
                ...
                try:
                    ...
                    logger.info(f"📊 Signal logged to database (ID: {signal_id})")
                ...
                try:
                    ...
                    if automation_saved:
                        ...
                        logger.warning(f"⚠️ Failed to queue signal for automation: {ticker}")
    ...
    def _format_signal_message(self, ticker: str, signal_strength: str, stars: str,
        ...
        lines = [
            ...
            f"📊 Strategy: {trade_rec.get('strategy', 'Unknown').replace('_', ' ')}",
        ...
        if technical_analysis and trading_style == 'INTRADAY':
            ...
            if today_context:
                ...
                gap_emoji = "📈" if gap_type == 'GAP_UP' else "📉" if gap_type == 'GAP_DOWN' else "➡️"
                momentum_emoji = "🚀" if intraday_momentum == 'BULLISH' else "📉" if intraday_momentum == 'BEARISH' else "🔄"
                volume_emoji = "🔥" if volume_ratio > 1.5 else "📊" if volume_ratio > 1.2 else "📉"
        ...
        if technical_analysis:
            ...
            bias_emoji = (
                "🚀" if market_bias == 'STRONG_BULLISH' else
                "🐂" if market_bias in ['BULLISH', 'LEAN_BULLISH'] else
                "📉" if market_bias == 'STRONG_BEARISH' else
            ...
            if entry_condition and entry_condition != 'Wait for confirmation':
                ...
                if trading_style == 'INTRADAY':
                    ...
                    lines.append(f"🎯 Entry Condition: {entry_condition}{timing_advice}")
                else:
                    lines.append(f"🎯 Entry Condition: {entry_condition}")
                lines.append("")
        
        lines.append("🎯 COMPLETE TRADE SETUP:")
        ...
        if option_legs:
            ...
            for i, leg in enumerate(option_legs, 1):
                ...
                tradingsymbol = leg.get('tradingsymbol', f"{ticker}30SEP{int(strike)}{option_type[0]}E")
                ...
                quality_emoji = "💎" if liquidity_score > 0.7 else "⭐" if liquidity_score > 0.4 else "⚠️"
        ...
        if technical_analysis:
            lines.append("📈 DETAILED TECHNICAL ANALYSIS:")
        ...
        lines.extend([
            "🔧 EXECUTION INFO:",
            f"📡 Data: {'Live Zerodha ✅' if data_source == 'zerodha_live' else 'Fallback ⚠️'}",
            f"📈 Technical: {'Enhanced ✅' if technical_available else 'Basic ⚠️'}",
        ])
        ...
        if zerodha_exec:
            ...
            lines.extend([
                f"💰 Estimated Margin: ₹{estimated_margin:,.0f}",
    ...
    def post_market_summary(self):
        ...
        try:
            ...
            if top_movers['gainers']:
                summary_lines.append("📈 Top Gainers: " + ", ".join(
                    [f"{m['ticker']} ({m['change']:+.1f}%)" for m in top_movers['gainers'][:3]]
                ))
            
            if top_movers['losers']:
                summary_lines.append("📉 Top Losers: " + ", ".join(
            ...
            if analysis_result:
                ...
                if self._should_send_signal(analysis_result, ticker):
                    ...
                    self.telegram_bot.send_message(
                        ...
                        f"✅ Signal met all quality thresholds and was sent above.\n"
                    ...
                    if confidence >= self.config.HIGH_CONFIDENCE_THRESHOLD:
                        ...
                        signal_strength = "🔔 MEDIUM SIGNAL (Manual)"
                        stars = "⭐⭐⭐⭐"
                    else:
                        signal_strength = "⚠️ WEAK SIGNAL (Manual)"
                        stars = "⭐⭐⭐"
                    ...
                    manual_header = [
                        ...
                        f"⚠️ Below Bot Threshold ({self.config.MIN_CONFIDENCE_SCORE:.0%}) - Manual Override",
                        "=" * 50,
                        ""
                    ]
                    ...
                    footer = [
                        ...
                        "",
                        "💡 This signal would NOT be sent automatically.",
                ...
                self.telegram_bot.send_message(f"❌ Could not analyze {ticker}")
                return None
   ...
   # === Helper Methods ===
    ...
    async def _analyze_overnight_gaps(self) -> Dict:
        ...
        try:
                    ...
                    if isinstance(data, pd.DataFrame) and len(data) >= 2:
                        ...
                        if abs(gap_pct) > 0.3:  # Only analyze meaningful gaps
                            try:
                                ...
                                if gap_pct > 1.0:
                                    confidence_indicator = "🎯" if enhanced_gap['confidence'] > 0.7 else "⚠️" if enhanced_gap['confidence'] > 0.5 else "❓"
            ...
            logger.info(f"✅ Enhanced gap analysis completed: {len(gap_analysis_results)} gaps analyzed, {significant_gap_count} significant")
    ...
    def _send_scan_summary(self, signals_found: int, errors: List[str], 
        ...
        if signals_found > 0 or errors or len(errors) > 2:  # Only send if significant activity
            ...
            summary_parts = [
                ...
                f"🎯 Avg Confidence: {filter_stats.get('avg_confidence', 0):.0%}",
            ]
            
            # Add filtering effectiveness
            signals_today = filter_stats.get('signals_today', 0)
            if signals_today > 0:
                summary_parts.append(f"📈 Today's Signals: {signals_today} (Quality filtered)")
        ...
        if signals_found > 0 or errors:
            ...
            if signals_found > 0:
                ...
                summary_parts.extend([
                    f"📈 Intraday: {intraday_count} | Swing: {swing_count}",
            ...
            if scan_results:
                ...
                for result in scan_results[:5]:  # Top 5
                    ...
                    style_emoji = "⚡" if trading_style == 'INTRADAY' else "📊"
        ...
        try:
            ...
            if self.market_status != expected_status:
                ...
                if expected_status == 'OPEN':
                    self.telegram_bot.send_message("🔔 Market opened - Bot status synchronized", silent=True)
                else:
            ...
            if stuck_positions:
                alert_msg = "⚠️ Long-held positions detected:\n"
...
def main():
   ...
   print("🚀 Starting Indian Trading Bot with Zerodha Kite Connect API...")
   print("=" * 60)
   ...
   try:
       ...
       if not os.getenv('ZERODHA_ACCESS_TOKEN'):
           if not (os.getenv('ZERODHA_API_KEY') and os.getenv('ZERODHA_API_SECRET')):
               print("❌ Missing Zerodha authentication!")
               print("Please provide either:")
               print("  - ZERODHA_ACCESS_TOKEN")
               print("  - ZERODHA_API_KEY and ZERODHA_API_SECRET")
               print("\n💡 Run 'python get_access_token.py' to generate access token")
               return
       ...
       if not os.getenv('ZERODHA_API_KEY'):
           print("❌ ZERODHA_API_KEY is required even with access token")
       ...
       if len(sys.argv) > 1:
           ...
           if market_status:
               print(f"✅ Zerodha API connected - Market status: {market_status.get('status', 'Unknown')}")
           else:
