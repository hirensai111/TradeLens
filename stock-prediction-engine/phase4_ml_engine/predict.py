...
class PredictionTool:
    ...
    def __init__(self):
        ...
        print("🚀 Phase 4 ML Prediction Engine")
        print("=" * 50)
    ...
    def predict_single_day(self, ticker: str, target_date: datetime) -> Dict:
        ...
        try:
            ...
            print(f"❌ Prediction failed for {target_date.date()}: {e}")
            return None
    ...
    def predict_multiple_days(self, ticker: str, start_date: datetime, end_date: datetime) -> List[Dict]:
        ...
        print(f"📈 Generating predictions from {start_date.date()} to {end_date.date()}...")
        ...
        while current_date <= end_date:
            ...
            if current_date.weekday() < 5:
                ...
                if prediction:
                    ...
                    print(f"   ✅ {current_date.date()}: ${prediction['predicted_close']:.2f} ({prediction['confidence']:.0%} confidence)")
                else:
                    print(f"   ❌ {current_date.date()}: Prediction failed")
    ...
    def format_prediction_report(self, predictions: List[Dict]) -> str:
        ...
        # Generate report
        report = f"""
🎯 STOCK PREDICTION REPORT - {ticker}
{'=' * 60}
  ...
  python predict.py TSLA tomorrow
    ...
    try:
        ...
        if start_date == end_date:
            ...
            if prediction:
                print(f"🎯 Prediction for {ticker} on {start_date.date()}:")
