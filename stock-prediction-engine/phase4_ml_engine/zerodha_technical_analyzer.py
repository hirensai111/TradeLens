...
class ZerodhaTechnicalAnalyzer:
    ...
    async def analyze_symbol_for_options(self, symbol: str, current_price: float, 
        ...
        try:
            ...
            # **FIXED: More lenient data validation** - Accept 5+ candles instead of 10+
            if len(df_daily) < 5:
                logger.warning(f"⚠️ Very limited daily data ({len(df_daily)} candles) for {symbol}")
                result['data_quality'] = 'VERY_LIMITED'
                # Don't give up completely, try to work with what we have
            elif len(df_daily) < 10:
                logger.warning(f"⚠️ Limited daily data ({len(df_daily)} candles) for {symbol}")
            ...
            try:
                ...
                logger.warning(f"⚠️ Today context analysis failed: {e}")
            ...
            try:
                ...
                logger.warning(f"⚠️ Momentum analysis failed: {e}")
            ...
            try:
                ...
                logger.warning(f"⚠️ Exit rules generation failed: {e}")
            ...
            try:
                ...
                logger.warning(f"⚠️ Options context analysis failed: {e}")
            ...
            try:
                ...
                logger.warning(f"⚠️ Market bias determination failed: {e}")
            ...
            if result['analysis_errors']:
                ...
                logger.warning(f"⚠️ Partial analysis completed for {symbol} with {len(result['analysis_errors'])} errors")
            else:
                logger.info(f"✅ Complete technical analysis for {symbol}: {result['market_bias']} bias")
            ...
            logger.error(f"❌ Complete technical analysis failure for {symbol}: {e}")
    ...
    def _get_zerodha_symbol(self, symbol: str) -> str:
        ...
        if symbol.upper() in symbol_mapping:
            ...
            print(f"⚠️ No mapping for {symbol}, using as-is: {mapped_symbol}")
    ...
    def get_historical_data_with_fallback(self, symbol: str, timeframe: str, days: int) -> pd.DataFrame:
        ...
        try:
            ...
            if not df.empty:
                logger.info(f"✅ Got {len(df)} {timeframe} candles for {symbol}")
                return df
            else:
                logger.warning(f"⚠️ Empty data for {symbol}, trying fallback")
    ...
    def _generate_fallback_data(self, symbol: str, timeframe: str, days: int) -> pd.DataFrame:
        ...
        print(f"📊 Generated {len(df)} synthetic {timeframe} candles for {symbol} (base: ₹{base_price})")
        return df
...
if __name__ == "__main__":
    print("🚀 Enhanced Zerodha Technical Analysis Engine")
    print("=" * 50)
    print("✅ Ready for integration with options trading bot")
    ...
    print("\n🎯 Key Enhancements:")
    print("  ⭐ _analyze_today_context() - Today's gap, momentum, volume")
    print("  ⭐ Enhanced _analyze_trend() - Intraday vs daily alignment")
    print("  ⭐ Enhanced _analyze_momentum() - 5-min RSI for intraday")
    print("  ⭐ Enhanced entry signals - Gap strategies, momentum trades")
    print("  ⭐ Enhanced options context - Strategy bias based on today's action")
    print("  ⭐ quick_intraday_analysis() - Immediate opportunity detection")