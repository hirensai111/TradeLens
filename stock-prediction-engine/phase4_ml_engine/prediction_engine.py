...
class PredictionErrorTracker:
    ...
    def validate_graph_analysis(self, graph_analysis):
        ...
        try:
            ...
            if not isinstance(graph_analysis, dict):
                print("   ❌ Graph analysis must be a dictionary")
                return False
            ...
            if not isinstance(reliability, (int, float)) or not (0 <= reliability <= 1):
                print("   ❌ pattern_reliability must be a number between 0 and 1")
                return False
            ...
            if not isinstance(breakout_analysis, dict):
                print("   ❌ breakout_analysis must be a dictionary")
                return False
            ...
            if not isinstance(strength, (int, float)) or not (0 <= strength <= 1):
                print("   ❌ breakout_strength must be a number between 0 and 1")
                return False
            ...
            if not isinstance(momentum_analysis, dict):
                print("   ❌ momentum_analysis must be a dictionary")
                return False
            
            if 'momentum_acceleration' not in momentum_analysis:
                print("   ❌ momentum_analysis missing momentum_acceleration")
                return False
            ...
            if not isinstance(acceleration, (int, float)) or not (0 <= acceleration <= 1):
                print("   ❌ momentum_acceleration must be a number between 0 and 1")
                return False
            ...
            if not isinstance(candlestick_analysis, dict):
                print("   ❌ candlestick_analysis must be a dictionary")
                return False
            ...
            for key in candlestick_required:
                if key not in candlestick_analysis:
                    print(f"   ❌ candlestick_analysis missing key: {key}")
                    return False
            ...
            if not isinstance(total_patterns, int) or total_patterns < 0:
                print("   ❌ total_patterns_found must be a non-negative integer")
                return False
            ...
            if not isinstance(nearest_support, (int, float)) or nearest_support <= 0:
                print("   ❌ nearest_support must be a positive number")
                return False
            
            if not isinstance(nearest_resistance, (int, float)) or nearest_resistance <= 0:
                print("   ❌ nearest_resistance must be a positive number")
                return False
            
            if nearest_support >= nearest_resistance:
                print("   ❌ nearest_support must be less than nearest_resistance")
                return False
            ...
            if not isinstance(level_quality, (int, float)) or not (0 <= level_quality <= 1):
                print("   ❌ level_quality must be a number between 0 and 1")
                return False
            
            print("   ✅ Graph analysis validation passed")
            return True
            
        except Exception as e:
            print(f"   ❌ Graph analysis validation error: {e}")
            return False
    ...
    def generate_default_graph_analysis(self, ticker, excel_data, market_data):
        ...
        print(f"🔧 Generating default graph analysis for {ticker}...")
        ...
        try:
            ...
            if abs(thirty_day_return) > 15 and volatility > 30:
                ...
                print(f"   🚀 Default breakout detected: {breakout_direction} (strength: {breakout_strength:.2f})")
            ...
            print(f"   ✅ Default graph analysis generated:")
            print(f"   📊 Pattern: {primary_pattern} (reliability: {pattern_reliability:.1%})")
            print(f"   🚀 Breakout: {breakout_direction} ({'detected' if breakout_detected else 'none'})")
            print(f"   ⚡ Momentum: {momentum_acceleration:.3f}")
            print(f"   📈 Support: ${nearest_support:.2f} | Resistance: ${nearest_resistance:.2f}")
            print(f"   ⚠️  Quality: BASIC (generated from limited data)")
            
            return default_graph_analysis
            
        except Exception as e:
            print(f"   ❌ Error generating default graph analysis: {e}")
            ...
            print(f"   ⚠️  Using minimal fallback graph analysis")
...
class MultiSourceNewsAnalyzer:
    ...
    def get_alpha_vantage_news(self, ticker):
        ...
        if not self.api_keys.get('alpha_vantage'):
            print("   ⚠️ Alpha Vantage API key not available")
    ...
    def get_finnhub_news(self, ticker):
        ...
        if not self.api_keys.get('finnhub'):
            print("   ⚠️ Finnhub API key not available")
            return {'sentiment': 0.0, 'articles': 0, 'confidence': 0.0}
        ...
        try:
            ...
            try:
                ...
                print("   ⚠️ TextBlob not available, using basic sentiment")
    ...
    def aggregate_multi_source_sentiment(self, ticker, phase3_data=None):
        ...
        print(f"   ✅ Multi-source aggregation complete:")
        print(f"   📊 Active sources: {active_sources}, Total articles: {total_articles}")
        print(f"   📈 Final sentiment: {final_sentiment:+.3f}, Confidence: {aggregate_confidence:.3f}")
...
class MarketRegimeDetector:
    ...
    def detect_market_regime(self, excel_data, market_data, news_data, graph_analysis):
        ...
        if news_data.get('graph_analysis_available', False):
            try:
                print("   📈 Integrating graph analysis into regime detection...")
                ...
                if primary_pattern in bullish_patterns:
                    ...
                    if base_technical_regime == 'mixed':
                        ...
                        print(f"   📈 Pattern override: {primary_pattern} → pattern_bullish")
                    ...
                    if base_technical_regime == 'mixed':
                        ...
                        graph_override_applied = True
                        print(f"   📉 Pattern override: {primary_pattern} → pattern_bearish")
                ...
                if breakout_detected:
                    if base_technical_regime == 'mixed' or pattern_technical_boost == 0:
                        ...
                        graph_override_applied = True
                        print(f"   🚀 BREAKOUT REGIME OVERRIDE applied")
                    else:
                        # Strengthen existing regime
                        graph_technical_strength += 0.4
                        print(f"   🚀 Breakout strengthens {graph_technical_regime} regime")
                ...
                print(f"   ⚠️ Graph analysis integration failed: {graph_error}")
                graph_technical_regime = base_technical_regime
        ...
        if news_data.get('breakout_detected', False) and graph_technical_strength > 0.6:
            ...
            print(f"   🚀 BREAKOUT REGIME: Technical breakout detected with strength {graph_technical_strength:.1f}")
        ...
        # PRIORITY 2: Strong Graph Pattern + Technical Alignment
            ...
            if graph_technical_regime in ['pattern_bullish', 'pattern_bearish']:
                ...
                print(f"   📈 PATTERN REGIME: {graph_technical_regime} with strength {graph_technical_strength:.1f}")
            else:
        ...
        if news_data.get('graph_analysis_available', False):
            ...
            if news_data.get('breakout_detected', False):
                ...
                print(f"   🚀 Breakout detection confidence boost: +0.12")
            
            # Pattern override confidence boost
            if graph_override_applied:
                graph_confidence_boost += 0.08
                print(f"   📈 Pattern override confidence boost: +0.08")
        ...
        print(f"   📊 Market regime detected: {main_regime}")
        print(f"   📊 Regime factors: Vol={volatility_regime}, Trend={trend_regime}, News={news_regime}, Tech={technical_regime}")
        print(f"   📊 Enhanced confidence: {enhanced_confidence:.3f} (base: {base_confidence:.3f}, graph boost: +{graph_confidence_boost:.3f})")
        if news_data.get('graph_analysis_available', False):
            print(f"   📈 Graph integration: Pattern={graph_pattern_regime}, Strength={graph_technical_strength:.2f}, Override={graph_override_applied}")
    ...
    def get_adaptive_weights(self, regime_data, prediction_days):
        ...
        print(f"   🎯 Calculating adaptive weights for {main_regime} regime...")
        ...
        print(f"   ✅ Adaptive weights: Mom={final_weights['momentum']:.3f}, Tech={final_weights['technical']:.3f}, News={final_weights['news']:.3f}")
...
class StockPredictionEngine:
    ...
    def load_excel_historical_data(self, ticker, excel_file_path=None):
        ...
        print(f"📊 Loading Excel analysis for {ticker}...")
        
        if excel_file_path is None:
            print("   ⚠️ No Excel file provided - using fallback data")
            return {
        ...
        try:
            ...
            if all_excel_data:
                ...
                print(f"   ✅ USING PROPER VOLATILITY: {proper_volatility:.2f}%")
                
                # Extract comprehensive data
                if loader.raw_data is not None and not loader.raw_data.empty:
                    raw_data = loader.raw_data
                    print(f"   📈 Historical data: {len(raw_data)} days")
                    ...
                    if 'Daily Change %' in raw_data.columns:
                        try:
                            ...
                            print(f"   ⚠️ Error processing daily changes: {e}")
                    ...
                    try:
                        ...
                        print(f"   ⚠️ Error processing high/low: {e}")
                ...
                if loader.technical_data is not None and not loader.technical_data.empty:
                    try:
                        ...
                        print(f"   ⚠️ Error processing technical data: {e}")
                ...
                if loader.summary_data is not None and not loader.summary_data.empty:
                    try:
                        ...
                        print(f"   ⚠️ Error processing summary: {e}")
                ...
                try:
                    ...
                    print(f"   ✅ 30-day performance = {thirty_day_performance:.2f}%")
                except Exception as e:
                    print(f"   ⚠️ Error getting 30-day performance: {e}")
                ...
                if loader.company_info is not None and not loader.company_info.empty:
                    try:
                        ...
                        print(f"   ⚠️ Error processing company info: {e}")
                ...
                print(f"✅ Excel analysis complete: {len(historical_analysis)} metrics extracted")
                print(f"📊 FINAL VOLATILITY: {historical_analysis['volatility']:.2f}%")
    ...
    def get_phase3_news_intelligence(self, ticker, graph_analysis=None):
        ...
        try:
            ...
            print(f"✅ Phase 3 data: {phase3_data['fresh_articles_found']} articles, {phase3_data['sentiment_1d']:+.3f} sentiment")
            
        except Exception as e:
            print(f"⚠️ Phase 3 system failed: {e}")
            phase3_data = {
        ...
        try:
            ...
            if multi_source_results['active_sources'] > 1:
                ...
                print(f"   📊 Enhanced sentiment: {enhanced_sentiment:+.3f} (from {multi_source_results['active_sources']} sources)")
                ...
                print(f"   ⚠️ Limited sources available, using Phase 3 primary")
            ...
            if graph_analysis:
                print(f"   📈 Integrating Graph Analysis insights...")
                ...
                try:
                    ...
                    if primary_pattern in bullish_patterns:
                        ...
                        print(f"   📈 Bullish pattern boost: +{pattern_boost:.3f} from {primary_pattern}")
                    elif primary_pattern in bearish_patterns:
                        ...
                        print(f"   📉 Bearish pattern penalty: {pattern_penalty:.3f} from {primary_pattern}")
                    ...
                    if breakout_detected and breakout_strength > 0.5:
                        ...
                        if breakout_direction == 'bullish':
                            ...
                            print(f"   🚀 BREAKOUT boost: +{breakout_boost:.3f} (strength: {breakout_strength:.3f})")
                            ...
                            print(f"   📉 BREAKDOWN penalty: {breakout_penalty:.3f} (strength: {breakout_strength:.3f})")
                    ...
                    if abs(graph_sentiment_adjustment) > 0.05:  # Only apply if meaningful
                        ...
                        print(f"   📊 Graph-enhanced sentiment: {enhanced_sentiment:+.3f} → {graph_enhanced_sentiment:+.3f}")
                        print(f"   📊 Graph weight: {graph_weight:.3f}, News weight: {news_weight:.3f}")
                    ...
                    print(f"   🎯 Graph confidence boost: +{graph_confidence_boost:.3f} → {enhanced_confidence:.3f}")
            ...
            print(f"✅ Multi-source news intelligence with GRAPH ANALYSIS complete:")
            print(f"   📊 Sources: {multi_source_results['active_sources']}, Articles: {multi_source_results['total_articles']}")
            print(f"   📈 Final sentiment: {graph_enhanced_sentiment:+.3f} (news: {enhanced_sentiment:+.3f})")
            print(f"   🎯 Final confidence: {enhanced_confidence:.3f}")
            if graph_analysis:
                print(f"   📈 Graph integration: ✅ Applied")
                print(f"   📊 Technical alignment: {graph_technical_alignment}")
                print(f"   📊 Pattern: {graph_analysis.get('pattern_detected', {}).get('primary_pattern', 'None')}")
                if graph_analysis.get('breakout_analysis', {}).get('breakout_detected', False):
                    print(f"   🚀 Breakout detected: {graph_analysis.get('breakout_analysis', {}).get('breakout_direction', 'unknown')}")
            
            return enhanced_news_data
            
        except Exception as e:
            print(f"❌ Multi-source aggregation failed: {e}")
    ...
    def enhance_news_with_custom_articles(self, news_data, custom_articles):
        ...
        if enhanced_news_data.get('multi_source_enabled', False):
            ...
            print(f"   🎯 Multi-source confidence boost: +{multi_source_boost + article_boost + custom_boost:.3f}")
        else:
        ...
        if enhanced_news_data.get('multi_source_enabled', False):
            ...
            print(f"   🎯 Multi-source signal strength: {enhanced_signal_strength:.3f}")
        
        # Final logging with multi-source details
        print(f"   ✅ Multi-source news enhancement complete:")
        print(f"   📊 Enhanced sentiment: {enhanced_sentiment:+.3f} (base: {base_sentiment:+.3f})")
        print(f"   📊 Custom impact: {avg_impact:+.2f}")
    ...
    def get_realtime_market_data(self, ticker, api_key):
        ...
        try:
            ...
            if "Global Quote" in data:
                ...
                print(f"   ⚠️ API issue: {data.get('Note', data.get('Error Message', 'Unknown error'))}")
                return None
        except Exception as e:
    ...
    def generate_multi_day_price_paths(self, current_price, volatility, prediction_days, num_paths=None, drift=None, overall_signal=0):
        ...
        if num_paths is None:
            ...
            print(f"   📊 Volatility {volatility:.1f}% → {vol_tier} tier → {num_paths:,} simulations")
        ...
        if drift is None:
            ...
            if prediction_days <= 5:
                ...
                if overall_signal > 0.15:  # Strong bullish - AMPLIFY
                    ...
                    print(f"   🚀 Strong bullish amplification: +{signal_drift_adjustment:.6f} drift, {drift_multiplier:.1f}x multiplier")
                    ...
                    print(f"   📉 Strong bearish amplification: {signal_drift_adjustment:.6f} drift, {drift_multiplier:.1f}x multiplier")
        ...
        print(f"   📊 Enhanced daily drift: {daily_drift:.6f} (signal: {overall_signal:+.3f})")
        ...
        print(f"   📊 Volatility tier limits: Short={base_limit_short:.3f}, Long={base_limit_long:.3f}, Adj={vol_adjustment:.1f}x")
            ...
            if num_paths >= 3000 and (path_num + 1) % convergence_check_interval == 0:
                if path_num >= 1500:  # Only check after minimum paths
                    ...
                    if convergence_ratio < convergence_threshold:
                        print(f"   ✅ Early convergence achieved at {path_num + 1:,} paths (threshold: {convergence_threshold:.1%})")
                        break
        ...
        print(f"   ✅ VOLATILITY-AWARE paths generated: {len(price_paths):,} paths")
        print(f"   📊 Expected return: {expected_return:+.2f}%")
        print(f"   📊 Path stats: Mean=${avg_final_price:.2f}, Range=${np.min(final_prices):.2f}-${np.max(final_prices):.2f}")
        print(f"   🎯 Precision: ±{confidence_interval_width:.2f}% (95% CI width)")
        print(f"   📈 Standard error: {std_error:.2f} ({std_error/avg_final_price*100:.3f}%)")
    ...
    def calculate_daily_stats_from_paths(self, price_paths, current_price, prediction_days, volatility):
        ...
        print("📊 Calculating daily statistics from price paths...")
    ...
    def perform_mathematical_analysis(self, excel_data, market_data, custom_articles, prediction_days, price_paths, graph_analysis):
        ...
        if graph_analysis:
            print(f"   📈 Integrating Graph Analysis momentum insights...")
            ...
            if graph_momentum_acceleration > 0.75:  # Very strong momentum
                ...
                print(f"   🚀 Strong graph momentum boost: +{graph_momentum_boost:.4f} from acceleration {graph_momentum_acceleration:.3f}")
                ...
                print(f"   📉 Graph momentum penalty: {graph_momentum_boost:.4f} from acceleration {graph_momentum_acceleration:.3f}")
        ...
        if graph_analysis:
            print(f"   📊 Integrating Graph Analysis technical insights...")
            ...
            try:
                ...
                if primary_pattern in bullish_patterns and pattern_reliability > 0.5:
                    ...
                    print(f"   📈 Bullish pattern boost: +{pattern_boost:.3f} from {primary_pattern} (reliability: {pattern_reliability:.3f})")
                    ...
                    print(f"   📉 Bearish pattern penalty: -{pattern_penalty:.3f} from {primary_pattern} (reliability: {pattern_reliability:.3f})")
                ...
                if breakout_detected and breakout_strength > 0.6:
                    if breakout_direction == 'bullish':
                        ...
                        print(f"   🚀 BREAKOUT detected: +{graph_breakout_signal:.3f} bullish signal (strength: {breakout_strength:.3f})")
                    elif breakout_direction == 'bearish':
                        graph_breakout_signal = -breakout_strength * 0.4
                        print(f"   📉 BREAKDOWN detected: {graph_breakout_signal:.3f} bearish signal (strength: {breakout_strength:.3f})")
                ...
                if nearest_support and nearest_resistance and level_quality > 0.6:
                    ...
                    if support_distance < 0.02:  # Very close to support (bullish)
                        ...
                        print(f"   📊 Near support level boost: +{level_boost:.3f}")
                        ...
                        print(f"   📊 Near resistance level penalty: -{level_penalty:.3f}")
                ...
                print(f"   ⚠️ Graph technical analysis integration failed: {graph_tech_error}")
                graph_technical_boost = 0.0
        ...
        if graph_analysis:
            print(f"   🎯 Applying graph analysis weight adjustments...")
            ...
            if regime_override_signal:
                ...
                if 'breakout_confirmed' in regime_override_signal:
                    ...
                    adaptive_weights['news'] *= 0.8
                    print(f"   📈 Pattern completion regime override applied")
            
            print(f"   📊 Graph boost factors: Momentum={momentum_boost_factor:.2f}x, Technical={technical_boost_factor:.2f}x")
            print(f"   📊 Weight changes: Mom {original_momentum_weight:.3f}→{adaptive_weights['momentum']:.3f}, Tech {original_technical_weight:.3f}→{adaptive_weights['technical']:.3f}")
        ...
        if prediction_days <= 5:
            if thirty_day_return > 15:  # Strong positive momentum
                ...
                print(f"   🚀 Momentum amplification: +{momentum_boost:.2f} weight boost for {thirty_day_return:.1f}% return")
                ...
                momentum_amplification_applied = True
                print(f"   📉 Bearish momentum amplification: +{momentum_boost:.2f} weight boost for {thirty_day_return:.1f}% return")
        ...
        print(f"   🎯 FINAL WEIGHTS ({prediction_days}d, {regime_data['main_regime']}): Mom={momentum_weight:.3f}, Tech={technical_weight:.3f}, News={news_weight:.3f}")
        ...
        if graph_analysis:
            ...
            print(f"   📊 Graph technical integration: pattern_adj={pattern_adjustment:+.3f}, breakout={graph_breakout_signal:+.3f}, boost={graph_technical_boost:+.3f}")
        ...
        if thirty_day_return > 20 and raw_signal > 0.1:  # Strong momentum amplification
            ...
            print(f"   🚀 Strong momentum amplification: {thirty_day_return:.1f}% 30d return")
            ...
            bearish_protection_applied = False
            print(f"   📉 Strong downtrend amplification: {thirty_day_return:.1f}% 30d return")
        elif abs(raw_signal) < 0.05:  # Weak signals
            signal_override_protection = 0.9   # Slight dampening
            bearish_protection_applied = False
            print(f"   🔧 Weak signal dampening applied")
        else:
        ...
        if abs(calibrated_signal) > abs(adjusted_signal) and calibration_results['strong_bullish_override']:
            print(f"   🔧 Using calibrated signal override: {calibrated_signal:+.3f} vs adjusted: {adjusted_signal:+.3f}")
        ...
        if graph_analysis:
            ...
            if (graph_analysis.get('breakout_analysis', {}).get('breakout_detected', False) and 
                ...
                if breakout_direction in ['bullish', 'bearish']:
                    ...
                    print(f"   🚀 STRONG BREAKOUT OVERRIDE: {original_signal:+.3f} → {overall_signal:+.3f} (breakout strength: {breakout_strength:.3f})")
        ...
        if prediction_days <= 5:
            ...
            if thirty_day_return > 25 and overall_signal > 0.2:  # EXCEPTIONAL bullish (like AMD)
                ...
                print(f"   🚀🚀 EXCEPTIONAL bullish momentum override: +5% BOOST (was reduction)")
                ...
                print(f"   🚀 Very strong bullish: NO drift reduction applied")
                ...
                print(f"   🚀 Strong bullish: Minimal 2% drift reduction")
                ...
                print(f"   📉 Very bearish signal: Strong drift reduction applied")
            ...
            if thirty_day_return > 20 and overall_signal > 0.2:  # Strong momentum long-term
                ...
                print(f"   🚀 Strong long-term bullish: NO drift reduction")
        ...
        if len(price_paths) > 0:
            ...
            if thirty_day_return > 25 and overall_signal > 0.2:
                ...
                print(f"   🚀🚀 Exceptional momentum confidence boost applied")
        ...
        if hasattr(self, '_last_news_data') and self._last_news_data:
            ...
            if self._last_news_data.get('graph_analysis_integrated', False):
                ...
                print(f"   🎯 Graph-enhanced news confidence: +{graph_confidence_boost:.3f}")
        ...
        if graph_analysis:
            ...
            if graph_confidence_boost > 0:
                ...
                enhanced_confidence = min(0.95, enhanced_confidence + graph_confidence_boost)
                print(f"   📈 Graph analysis confidence boost: {original_confidence:.3f} → {enhanced_confidence:.3f} (+{graph_confidence_boost:.3f})")
        ...
        print(f"   ✅ MAE-ENFORCED analysis complete with VOLATILITY-AWARE CONFIDENCE and GRAPH ANALYSIS")
        print(f"   📊 Market regime: {regime_data['main_regime']} (confidence: {regime_data['regime_confidence']:.3f})")
        print(f"   📊 Overall signal: {overall_signal:+.3f} (raw: {raw_signal:+.3f})")
        print(f"   📊 Expected return: {analysis_results['statistical_metrics']['expected_return_pct']:+.2f}%")
        print(f"   📊 Drift reduction: {drift_reduction:.3f} (override: {momentum_override})")
        print(f"   📊 Drift confidence: {drift_confidence:.1%}")
        print(f"   🎯 VOLATILITY-AWARE Confidence: {enhanced_confidence:.1%} ({confidence_results.get('volatility_tier', 'Unknown')} volatility tier)")
        print(f"   🎯 MAE Performance: {mae_info['final_day_mae']:.1f}% vs target {mae_info['mae_target']:.1f}% ({'✅' if mae_info['mae_target_met'] else '❌'})")
        print(f"   🎯 MAE Success Rate: {mae_info['mae_success_rate']:.1%} across {len(daily_stats) if 'daily_stats' in locals() else 0} days")
        
        if graph_analysis:
            print(f"   📈 Graph Analysis Integration: ✅ APPLIED")
            print(f"   📊 Graph momentum boost: {graph_momentum_boost:+.4f}")
            print(f"   📊 Graph technical boost: {graph_technical_boost:+.3f}")
            print(f"   📊 Graph breakout signal: {graph_breakout_signal:+.3f}")
    ...
    def enhanced_directional_bias_calibration(self, excel_data, market_data, custom_articles):
        ...
        print("🎯 Applying enhanced directional bias calibration...")
        ...
        if thirty_day_return > 25:  # Very strong momentum
            print(f"   🚀🚀 VERY strong bullish override: {thirty_day_return:.1f}% 30d return")
            ...
            print(f"   🚀 Strong bullish override triggered: {thirty_day_return:.1f}% 30d return")
            ...
            print(f"   📈 Moderate bullish override: {thirty_day_return:.1f}% 30d return")
        ...
        # Rule 2: Enhanced moderate bullish nudge (lowered threshold)
            ...
            print(f"   📈 Moderate bullish nudge: {thirty_day_return:.1f}% 30d return")
        ...
        # NEW Rule 2.5: Handle strong bearish momentum
            ...
            print(f"   📉 Strong bearish override: {thirty_day_return:.1f}% 30d return")
            base_signal = min(-0.1, max(-0.25, momentum_score * 6))  # Amplify bearish signals
        elif thirty_day_return < -8:  # Moderate bearish momentum
            print(f"   📉 Moderate bearish override: {thirty_day_return:.1f}% 30d return")
        ...
        if current_price > 0 and sma_20 > 0:
            ...
            if price_above_sma > 0.05 and thirty_day_return > 5:  # Price well above SMA + positive momentum
                ...
                print(f"   📊 Technical momentum alignment: +{technical_boost:.3f} (price {price_above_sma*100:+.1f}% above SMA20)")
        ...
        if abs(calibrated_signal) > 0.5:
            ...
            print(f"   🚀 Strong signal boost applied: +{signal_boost:.3f}")
        ...
        print(f"   ✅ Enhanced calibrated signal: {calibrated_signal:+.3f} (enhanced: {enhanced_signal:+.3f})")
    ...
    def enhanced_drift_scaling(self, base_drift, momentum_score, volatility, news_momentum, recent_performance):
        ...
        if momentum_score > 0.01:  # Bullish momentum
            if momentum_score > 0.05:  # VERY strong momentum (>60% annualized)
                ...
                print(f"   🚀🚀 VERY strong bullish momentum: {abs(momentum_score):.3f} → {momentum_amplification:.1f}x")
                ...
                print(f"   🚀 Strong bullish momentum: {abs(momentum_score):.3f} → {momentum_amplification:.1f}x")
            ...
            if momentum_score < -0.05:  # VERY strong bearish
                ...
                print(f"   📉📉 VERY strong bearish momentum: {abs(momentum_score):.3f} → {momentum_amplification:.1f}x")
            elif momentum_score < -0.02:  # Strong bearish
                momentum_amplification = 1.0 + min(3.5, abs(momentum_score) * 18)  # Increased from 16 to 18
                print(f"   📉 Strong bearish momentum: {abs(momentum_score):.3f} → {momentum_amplification:.1f}x")
        ...
        if abs(recent_performance) > 20:  # Exceptional performance (like AMD's 26.92%)
            ...
            print(f"   🎯🎯 Exceptional performance boost: {recent_performance:.1f}% → {performance_amplification:.1f}x")
            ...
            print(f"   🎯 Strong performance boost: {recent_performance:.1f}% → {performance_amplification:.1f}x")
        ...
        if hit_cap:
            print(f"   ⚠️ Drift near/at cap: {enhanced_drift:.1f}% (cap: ±{max_drift:.1f}%)")
        ...
        print(f"   ✅ IMPROVED drift: {enhanced_drift:.2f}% (base: {base_drift:.2f}%, multiplier: {drift_multiplier:.2f}x)")
        print(f"   📊 Momentum tier: {drift_scaling_results['momentum_tier']}, Amplification: {momentum_amplification:.1f}x")
    ...
    def enhanced_range_calibration(self, mean_price, current_price, volatility, ticker, prediction_day):
        ...
        print(f"📊 Applying MAE-enforced range calibration for day {prediction_day}...")
        ...
        print(f"   🎯 MAE target: {target_mae:.1%} ({volatility_tier} volatility tier)")
        ...
        print(f"   📊 Volatility tier: {volatility_tier} → {base_multiplier}x multiplier (was 2.2x)")
        ...
        print(f"   🎯 MAE enforcement: {initial_range_pct:.1%} → {compressed_range_pct:.1%}")
        print(f"   📊 Compression: {compression_results['total_compression']:.3f}x")
        ...
        if is_tech_stock and volatility > 40:
            ...
            print(f"   🚀 Tech buffer: -{tech_buffer:.2f} (reduced)")
        ...
        # Enhanced logging
        status_emoji = "✅" if mae_success else "❌"
        print(f"   📈 Final Range: ${enhanced_low:.2f} - ${enhanced_high:.2f}")
        print(f"   🎯 MAE Result: {final_mae_pct:.1f}% {status_emoji} (target: {target_mae*100:.1f}%)")
        print(f"   📊 Range tightening: {range_calibration_results['range_tightening_pct']:.1f}%")
    ...
    def enhanced_confidence_integration(self, signal_strength, news_confidence, drift_factor, volatility, prediction_days, monte_carlo_paths=1000, total_news_articles=0):
        ...
        print(f"🎯 Calculating volatility-aware confidence for {volatility:.1f}% volatility...")
        ...
        print(f"   📊 Volatility tier: {vol_tier} → Confidence range: {min_confidence}-{max_confidence}%")
        ...
        if abs(signal_strength) > 0.4 and volatility < 60:
            ...
            print(f"   🎯 Strong signal offset: -{signal_vol_offset:.1f} volatility penalty")
        else:
        ...
        if volatility > 60 and abs(signal_strength) < 0.2:
            ...
            print(f"   ⚠️ Extreme volatility + weak signal penalty: -{extreme_penalty}")
        ...
        if prediction_days <= 2 and volatility > 45:
            ...
            print(f"   ⚠️ Short-term high volatility penalty: -{short_term_vol_penalty}")
        ...
        print(f"   ✅ Volatility-aware confidence: {final_confidence:.1f}%")
        print(f"   📊 Components: Signal={signal_component:.1f}, News={news_component:.1f}, Drift={drift_component:.1f}")
        print(f"   📊 Penalties: Vol={volatility_penalty:.1f}, Time={time_penalty:.1f}")
        print(f"   📊 Bonuses: Data Quality={data_quality_bonus:.1f}")
    ...
    def analyze_options_scenarios(self, current_price, daily_stats, options_data):
        ...
        print("📊 Analyzing options trading scenarios...")
        ...
        print(f"   ✅ Analyzed {len(scenarios)} options scenarios")
        return scenarios
    ...
    def calculate_daily_stats_from_paths_enhanced(self, price_paths, current_price, prediction_days, volatility, ticker):
        ...
        print("📊 Calculating enhanced daily statistics with MAE enforcement...")
        ...
        print(f"   ✅ MAE enforcement complete: {mae_success_count}/{total_days} days meet target ({mae_success_rate:.1f}%)")
        print(f"   📊 Average MAE: {avg_mae:.1f}% vs Target: {avg_target:.1f}%")
        print(f"   📊 Average range tightening: {avg_tightening:.1f}%")
        print(f"   🎯 Volatility tier: {volatility_tier}")
    ...
    def create_fallback_analysis(self, ticker, excel_data, news_data, market_data, prediction_days, graph_analysis):
        ...
        print(f"✅ Enhanced fallback analysis complete")
        print(f"   📊 Expected return: {enhanced_expected_return:+.2f}% (base: {base_expected_return:+.2f}%)")
    ...
    def generate_enhanced_price_paths(self, current_price, volatility, prediction_days, enhanced_drift, overall_signal, num_paths=1000):
        ...
        print(f"   📊 Enhanced daily drift: {adjusted_daily_drift:.6f}, volatility: {daily_vol:.4f}")
    ...
    def analyze_with_claude_ultimate_enhanced(self, ticker, excel_data, news_data, market_data, custom_articles, graph_analysis=None, prediction_days=1, options_data=None):        
        ...
        try:
            ...
            return self.create_fallback_analysis(ticker, excel_data, news_data, market_data, prediction_days)
        except AttributeError:
            print("❌ Claude API key not found - check self.claude_key")
            return self.create_fallback_analysis(ticker, excel_data, news_data, market_data, prediction_days)
        ...
        print(f"   📊 Volatility: {volatility:.1f}% → Applying volatility-aware confidence system")
        if graph_analysis:
            print(f"   📈 Graph Analysis: ✅ Available → Integrating technical patterns")
        ...
        if graph_analysis:
            ...
            if graph_momentum_acceleration > 0.75:
                ...
                print(f"   📈 Graph momentum boost applied: +{momentum_graph_boost:.3f} from acceleration {graph_momentum_acceleration:.3f}")
        ...
        try:
            ...
            print(f"   ⚠️ Mathematical analysis failed: {e}, using fallback signal calculation")
            overall_signal = 0.0
        ...
        try:
            ...
            print(f"   ⚠️ Enhanced price paths failed: {e}, using fallback Monte Carlo")
            try:
                ...
                print("   ✅ Using volatility-aware Monte Carlo price path generation")
            except Exception as e2:
                print(f"   ❌ All price path generation failed: {e2}")
                return self.create_fallback_analysis(ticker, excel_data, news_data, market_data, prediction_days)
        ...
        print(f"   📊 Monte Carlo paths: {monte_carlo_paths:,} (volatility-adjusted)")
        ...
        try:
            ...
            print(f"   ❌ Daily stats calculation failed: {e}")
            return self.create_fallback_analysis(ticker, excel_data, news_data, market_data, prediction_days)
        ...
        try:
            ...
            print(f"   ❌ Final mathematical analysis failed: {e}")
            return self.create_fallback_analysis(ticker, excel_data, news_data, market_data, prediction_days)
        ...
        print(f"   🎯 VOLATILITY-AWARE Confidence: {enhanced_confidence:.1%}")
        print(f"   📊 Volatility tier: {confidence_results.get('volatility_tier', 'Unknown')}")
        print(f"   📊 Confidence factors: Signal={confidence_results.get('signal_component', 0):.1f}, Vol penalty={confidence_results.get('volatility_penalty', 0):.1f}")
...
- Breakout Status: {'✅ ' + breakout_direction.upper() + ' BREAKOUT' if breakout_detected else '❌ No breakout'} (Strength: {breakout_strength:.1%})
...
- Momentum Boost: {'✅' if integration_results.get('momentum_boost_applied', False) else '❌'}
- Technical Boost: {'✅' if integration_results.get('technical_boost_applied', False) else '❌'}  
- Breakout Override: {'✅' if integration_results.get('breakout_override_applied', False) else '❌'}
- Pattern Signal: {'✅' if integration_results.get('pattern_signal_applied', False) else '❌'}
- Regime Override: {'✅ ' + str(graph_analysis.get('integration_signals', {}).get('regime_override_signal', '')) if integration_results.get('regime_override_applied', False) else '❌'}
- Confidence Boost: {'✅' if integration_results.get('confidence_boost_applied', False) else '❌'}
"""
        else:
...
VOLATILITY-AWARE SYSTEM: ✅ Applied (Tier: {volatility_tier}, Penalty: -{volatility_penalty:.1f})
FRESH SEARCH INTEGRATION: {"✅ Applied" if news_data and news_data.get('fresh_search_performed', False) else "❌ Not Available"}
GRAPH ANALYSIS INTEGRATION: {"✅ Applied" if graph_integration_applied else "❌ Not Available"}
...
IMPORTANT: Your predictions MUST reflect the ENHANCED confidence of {enhanced_confidence:.1%} (not generic 85%) and incorporate the graph analysis insights."""
        ...
        try:
            ...
            if json_match:
                ...
                if abs(analysis.get('total_expected_return_pct', 0) - expected_return) > 2:
                    print(f"   🔧 Overriding Claude's return prediction: {analysis.get('total_expected_return_pct', 0):.2f}% → {expected_return:.2f}%")
                ...
                if abs(analysis.get('final_target_price', 0) - daily_stats[-1]['mean_price']) > current_price * 0.02:
                    print(f"   🔧 Overriding Claude's target price: ${analysis.get('final_target_price', 0):.2f} → ${daily_stats[-1]['mean_price']:.2f}")
                ...
                if abs(analysis.get('confidence', 0.85) - enhanced_confidence) > 0.05:
                    print(f"   🔧 Enforcing volatility-aware confidence: {analysis.get('confidence', 0.85):.3f} → {enhanced_confidence:.3f}")
                ...
                # Enhanced logging
                print(f"✅ ENHANCED Claude analysis complete with VOLATILITY-AWARE confidence system and GRAPH ANALYSIS")
                print(f"   📊 Signal: {overall_signal:+.3f}, Expected Return: {expected_return:+.2f}%")
                print(f"   🔄 Fresh Articles: {news_data.get('fresh_articles_found', 0) if news_data else 0}")
                print(f"   🎯 VOLATILITY-AWARE Confidence: {enhanced_confidence:.1%} ({volatility_tier} tier)")
                print(f"   📊 Monte Carlo paths: {monte_carlo_paths:,} (volatility-adjusted)")
                if graph_analysis:
                    ...
                    print(f"   📊 Technical Alignment: {news_data.get('graph_technical_alignment', 'neutral') if news_data and news_data.get('graph_analysis_integrated') else 'neutral'}")
                
                return analysis
                
            else:
                print("⚠️ Could not parse Claude JSON response")
                return self.create_fallback_analysis(ticker, excel_data, news_data, market_data, prediction_days)
    ...
    def _calculate_data_quality_metrics(self, news_data, custom_articles, excel_data):
        ...
        try:
            ...
            print(f"   ⚠️ Data quality calculation error: {e}")
        ...
        try:
            ...
            if hasattr(self, 'error_tracker') and prediction_data['mae_performance']['system_enabled']:
                try:
                    ...
                    print(f"   ⚠️ MAE tracking setup failed: {e}")
            
            # Enhanced logging with MAE details
            mae_info = prediction_data['mae_performance']
            print(f"✅ Prediction data saved successfully:")
            print(f"   📄 File: {json_filename}")
            print(f"   🎯 MAE System: {'✅ Enabled' if mae_info['system_enabled'] else '❌ Disabled'}")
            if mae_info['system_enabled']:
                print(f"   📊 Final MAE: {mae_info['final_day_mae']:.1f}% (target: {mae_info['mae_target']:.1f}%)")
                print(f"   📈 Success Rate: {mae_info['mae_success_rate']:.1%}")
                print(f"   🏷️ Volatility Tier: {mae_info['volatility_tier']}")
            print(f"   🎯 Confidence: {prediction_data['quality_metrics']['prediction_confidence']:.1%}")
            print(f"   📊 Monte Carlo Paths: {prediction_data['analysis_metadata']['monte_carlo_paths']:,}")
...
if __name__ == "__main__":
    ...
    print("🚀 Stock Prediction Engine - FIXED VERSION")
    print("=" * 50)
    ...
    print("\n✅ Fixed Stock Prediction Engine ready for use!")