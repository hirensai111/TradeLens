...
class TelegramSignalBot:
    ...
    def format_position_update(self, position: Dict) -> str:
        ...
        if position.get('action'):
            action_emoji = "🔔" if position['action'] == 'HOLD' else "⚠️"
    ...
    def send_market_alert(self, alert_type: str, details: Dict = None) -> bool:
        ...
        alert_messages = {
            ...
            'high_volatility': "⚠️ **HIGH VOLATILITY** - VIX spike detected",
            'news_alert': "📰 **BREAKING NEWS** - Market moving event",
            'technical_breakout': "🔥 **TECHNICAL BREAKOUT** - Major level broken",
            'volume_spike': "📊 **VOLUME SPIKE** - Unusual activity detected",
            'market_open': "🔔 **MARKET OPEN** - Trading session started",
            'market_close': "🔔 **MARKET CLOSE** - Trading session ended",
    ...
    def send_daily_summary(self, summary_data: Dict) -> str:
        ...
        if 'win_rate' in summary_data:
            lines.append(f"📈 Win Rate: {summary_data['win_rate']:.1f}%")
        ...
        if 'worst_performer' in summary_data:
            lines.extend([
                "",
                "**Worst Performer:**",
                f"📉 {summary_data['worst_performer']['ticker']}: "
...
if __name__ == "__main__":
    ...
    if bot_token and chat_id:
        ...
        try:
            ...
            # Test basic message
            if bot.send_message("✅ Telegram bot is working! This is a test message."):
            ...
            if bot.send_market_alert('market_open', {'index': 'NIFTY50', 'opening': 19245.50}):
                print("✅ Market alert test successful!")
