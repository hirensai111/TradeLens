...
from options_analyzer.brokers.zerodha_technical_analyzer import ZerodhaTechnicalAnalyzer, integrate_technical_analysis
...
INDIAN_MARKET_HOURS = 6.25  # Trading hours per day
US_MARKET_HOURS = 6.5
TRADING_DAYS_PER_YEAR = 252
INDIAN_RISK_FREE_RATE = 0.065  # 6.5% current rate
...
LOT_SIZE_TCS = 175
LOT_SIZE_INFY = 400
LOT_SIZE_HDFCBANK = 550
LOT_SIZE_BAJFINANCE = 750
LOT_SIZE_MARUTI = 50
LOT_SIZE_HINDUNILVR = 300
LOT_SIZE_HCLTECH = 350
LOT_SIZE_MPHASIS = 275
LOT_SIZE_BHARTIARTL = 475

# Risk Limits
MAX_POSITION_VALUE = 500000  # ₹5 lakh
MAX_SINGLE_LOSS = 25000  # ₹25k
MAX_DAILY_LOSS = 50000  # ₹50k
MAX_PORTFOLIO_DELTA = 50
MAX_PORTFOLIO_GAMMA = 10
MARGIN_BUFFER = 0.20  # 20%

# Option Analysis Thresholds
MIN_PREMIUM_THRESHOLD = 0.50  # Minimum ₹0.50 premium
MIN_OI_THRESHOLD = 100  # Minimum open interest
MIN_VOLUME_THRESHOLD = 50  # Minimum volume
MAX_SPREAD_PERCENT = 0.05  # 5% max bid-ask spread

# Time Decay Factors
INTRADAY_TIME_DECAY_FACTOR = 1.5
WEEKLY_EXPIRY_BOOST = 1.3

# Greeks Risk Weights (for risk score calculation)
INTRADAY_DELTA_WEIGHT = 0.4
INTRADAY_GAMMA_WEIGHT = 15
INTRADAY_VEGA_WEIGHT = 1
INTRADAY_THETA_WEIGHT = 10

SWING_DELTA_WEIGHT = 0.3
SWING_GAMMA_WEIGHT = 10
SWING_VEGA_WEIGHT = 2
SWING_THETA_WEIGHT = 5

# API Rate Limits
ZERODHA_CALLS_PER_SECOND = 3
ZERODHA_HISTORICAL_CALLS_PER_SECOND = 2
# ==================================================
...
def convert_to_serializable(obj):
    """Convert numpy/pandas objects to JSON serializable types with safety checks"""
...
class NumpyEncoder(json.JSONEncoder):
    """Custom JSON encoder for numpy and pandas types with safety checks"""
...
class ZerodhaEnhancedOptionsCalculator:
    ...
    def _initialize_zerodha_data(self):
        ...
        try:
            ...
            default_lot_sizes = {
                'NIFTY 50': 75,           # [OK] Corrected
                'RELIANCE': 500,          # [OK] Corrected
                'HDFCBANK': 550,          # [OK] Already correct
                'TCS': 175,               # [OK] Corrected
                'INFY': 400,              # [OK] Corrected
                'BAJFINANCE': 750,        # [OK] Added
                'MARUTI': 50,              # [OK] Added
                'HINDUNILVR': 300,        # ADD
                'HCLTECH': 350,           # ADD
                'MPHASIS': 275,           # ADD
            ...
            for symbol in default_lot_sizes.keys():
                ...
                logger.info(f"[OK] {symbol}: Lot size {self.lot_sizes[symbol]}")
    ...
    def calculate_complete_greeks(self, S: float, K: float, T: float, r: float,
        ...
        if not 0 <= dividend_yield <= 1:
            raise ValueError(f"Dividend yield must be 0-100%, got {dividend_yield}")
        ...
        # Adjust for Indian market hours
        T_adjusted = T * (INDIAN_MARKET_HOURS / US_MARKET_HOURS)
        ...
        # Risk score calculation using defined weights
        if is_intraday:
            risk_score = (abs(delta) * INTRADAY_DELTA_WEIGHT +
                         abs(gamma) * INTRADAY_GAMMA_WEIGHT +
                         abs(vega) * INTRADAY_VEGA_WEIGHT +
                         abs(theta) * INTRADAY_THETA_WEIGHT)
        else:
            risk_score = (abs(delta) * SWING_DELTA_WEIGHT +
                         abs(gamma) * SWING_GAMMA_WEIGHT +
                         abs(vega) * SWING_VEGA_WEIGHT +
                         abs(theta) * SWING_THETA_WEIGHT)
...
class ZerodhaMarketDataProvider:
    """Market data provider using Zerodha Kite Connect with rate limiting"""
    ...
    async def fetch_live_data(self, symbol: str) -> Dict:
        """Fetch live market data from Zerodha with proper async handling and rate limiting"""
        try:
            zerodha_symbol = self.get_zerodha_symbol(symbol)

            # Run sync Zerodha API calls in thread pool to avoid blocking
            ...
            # Apply rate limiting for historical data calls
            await self.historical_rate_limiter.acquire()

            # Get historical data for volatility calculation (run in parallel)
            historical_df, intraday_df = await asyncio.gather(
                loop.run_in_executor(
                    None, self.zerodha.get_historical_data, zerodha_symbol, 'day', 30
                ),
                loop.run_in_executor(
                    None, self.zerodha.get_historical_data, zerodha_symbol, '5minute', 1
                )
            )
            
            # Safely extract and validate price data
            ...
            result = {
                ...
                'close': float(quote_data.get('close', current_price)),
                'volume': int(quote_data.get('volume', 0)),
                'change': float(quote_data.get('change', 0)),
                'change_percent': float(quote_data.get('change_percent', 0)),
                'bid': float(quote_data.get('bid', current_price)),
                'ask': float(quote_data.get('ask', current_price)),
                'oi': int(quote_data.get('oi', 0) or quote_data.get('open_interest', 0)),
            ...
            logger.info(f"[OK] Live data fetched for {symbol}: ₹{result['current_price']}")
            return result
...
class ZerodhaOptionsChainProvider:
    ...
    def _create_zerodha_option_symbol(self, symbol: str, strike: float, expiry: str, option_type: str) -> str:
        ...
        try:
            ...
            # [TARGET] CRITICAL FIX: Correct date format based on real Zerodha data
            if is_index:
                ...
                # [WRENCH] KEY FIX: Remove the leading zero from month_day if month is single digit
            ...
            print(f"[TARGET] Generated symbol: {zerodha_symbol} (Expiry: {expiry}, Type: {'Index' if is_index else 'Stock'})")
            return zerodha_symbol
            
        except Exception as e:
            logger.error(f"Error creating option symbol for {symbol} {strike} {expiry} {option_type}: {e}")
            
            # Fallback format (October 28, 2025 expiry)
            option_suffix = 'CE' if option_type.upper() in ['CALL', 'CE'] else 'PE'

            if symbol.upper() in ['NIFTY', 'NIFTY50', 'NIFTY 50']:
                return f"NIFTY251028{int(strike)}{option_suffix}"
            elif symbol.upper() in ['BANKNIFTY', 'NIFTY BANK']:
                return f"BANKNIFTY251028{int(strike)}{option_suffix}"
            else:
                return f"{symbol.upper()}28OCT{int(strike)}{option_suffix}"
    ...
    def _generate_strikes(self, symbol: str, spot: float) -> List[float]:
        ...
        # [TARGET] NEW APPROACH: Get actual available strikes from Zerodha data
        ...
        if actual_strikes:
            ...
            if reasonable_strikes:
                print(f"[TARGET] Using {len(reasonable_strikes)} actual strikes from Zerodha")
    ...
    def _get_available_strikes_from_zerodha(self, symbol: str) -> List[float]:
        ...
        try:
            ...
            print(f"[CHART] Found {len(strikes)} actual strikes for {symbol} expiry {expiry_date}")
    ...
    def get_appropriate_expiry(self, symbol: str) -> str:
        """Get appropriate expiry based on symbol type - UPDATED for October 2025"""
        today = datetime.now()

        # Determine if it's an index or stock
        is_index = symbol.upper() in ['NIFTY', 'NIFTY50', 'NIFTY 50', 'BANKNIFTY', 'NIFTY BANK', 'FINNIFTY']

        if today.year == 2025 and today.month == 10:
            if is_index:
                # For indices, check for nearest weekly/monthly expiry
                if today.day <= 28:
                    return "2025-10-28"  # October monthly expiry
                else:
                    return "2025-11-27"  # November monthly expiry
            else:
                # For stocks, use monthly expiry
                if today.day <= 28:
                    return "2025-10-28"  # October monthly expiry
                else:
                    return "2025-11-27"  # November monthly expiry

        # Fallback to October monthly expiry
        return "2025-10-28"

    def _get_next_expiry(self) -> str:
        """Get next expiry date - UPDATED for October 2025"""
        today = datetime.now()

        # October 2025 monthly expiry is on 28th
        if today.year == 2025 and today.month == 10:
        ...
        # Default to October monthly expiry
        return "2025-10-28"  # October monthly expiry for stocks
    ...
    async def fetch_option_chain(self, symbol: str, expiry_date: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            # [TARGET] CRITICAL FIX: Use appropriate expiry for each symbol type
            if not expiry_date:
                expiry_date = self.get_appropriate_expiry(symbol)
            
            print(f"[TARGET] Using expiry: {expiry_date} for {symbol}")
        ...
        try:
            ...
            for call in option_chain['calls']:
                ...
                if symbol in real_quotes and real_quotes[symbol].get('price', 0) > 0:
                    ...
                    logger.info(f"[OK] Enhanced {symbol} with real data: ₹{real_data.get('price')}")
    ...
    async def place_options_order(self, leg: OptionsLeg, dry_run: bool = True) -> Dict:
        ...
        try:
            ...
            if result['status'] == 'success':
                ...
                logger.info(f"[OK] Order placed: {leg.action} {leg.contracts}x{leg.lot_size} {leg.tradingsymbol} @ ₹{order_params['price']}")
                ...
                logger.error(f"[ERROR] Order failed: {result['message']}")
                return {
                    'status': 'error',
            ...
            for order_id, order_info in self.active_orders.items():
                ...
                if zerodha_order:
                    ...
                    if old_status != new_status:
                        ...
                        logger.info(f"[UP] Order update: {order_id} {old_status} → {new_status}")
    ...
    async def square_off_position(self, tradingsymbol: str, quantity: int) -> Dict:
        ...
        try:
            ...
            if result.get('status') == 'success':
                logger.info(f"[OK] Square off order placed: {result.get('order_id')}")
            else:
                logger.error(f"[ERROR] Square off failed: {result.get('message')}")
...
class ZerodhaEnhancedOptionsAnalyzer:
    ...
    def __init__(self, zerodha_client: ZerodhaAPIClient = None, claude_api_key: str = None):
        print("[ROCKET] Initializing Zerodha-Enhanced Options Analyzer v4.0...")
        ...
        if zerodha_client:
            ...
            try:
                ...
                print("[OK] Zerodha client initialized from environment")
            except Exception as e:
                print(f"[ERROR] Failed to initialize Zerodha client: {e}")
        ...
        print(f"[CHART] Market Status: {market_status.get('status', 'unknown')}")
        
        # Load Zerodha-specific data
        self._initialize_zerodha_integration()
        
        print("[OK] All components initialized with Zerodha integration")
    ...
    def _initialize_zerodha_integration(self):
        ...
        try:
            ...
            if margins:
                ...
                print(f"[MONEY] Available Cash: ₹{available_cash:,.2f}")
            ...
            if test_quotes:
                print(f"[UP] Live Data Test: {len(test_quotes)} quotes received")
    ...
    async def analyze_trade(self, symbol: str, trading_style: str = 'swing',
        ...
        try:
            ...
            print(f"[CHART] Current Price: ₹{market_data['current_price']:.2f} (Source: {market_data['data_source']})")
            
            # 2. Calculate volatility from Zerodha historical data
            volatility = self._calculate_zerodha_volatility(market_data)
            print(f"[UP] Historical Volatility: {volatility:.1f}%")
            ...
            try:
                ...
                print(f"[UP] Technical Analysis: {technical_analysis['market_bias']} bias, {technical_analysis['confidence_score']:.1%} confidence")
                print(f"[TARGET] Entry Signal: {technical_analysis['entry_signal']['signal_type']} - {technical_analysis['entry_signal']['reason']}")
                
            except Exception as tech_error:
                logger.error(f"[WARNING] Technical analysis failed: {tech_error}")
            ...
            if execute_trades and risk_check.get('approved', False):
                print("[ROCKET] Executing trades via Zerodha...")
                for leg in option_legs:
                    try:
                        ...
                        if result['status'] == 'success':
                            print(f"[OK] Order placed: {leg.tradingsymbol}")
                        else:
                            print(f"[ERROR] Order failed: {leg.tradingsymbol} - {result.get('message', 'Unknown error')}")
                ...
                print("[WARNING] Trades not executed - risk check failed")
            ...
            confidence_emoji = "[TARGET]" if strategy['confidence'] > 0.8 else "[OK]" if strategy['confidence'] > 0.6 else "[WARNING]"
            ...
            print(f"[TARGET] Entry: {entry_condition}")
            
            # Add summary stats
            print(f"[CHART] Summary: {len(option_legs)} legs, ₹{self._estimate_total_margin(option_legs):,.0f} estimated margin")
            ...
            print(f"[ERROR] Analysis failed for {symbol}: {str(e)}")
    ...
    async def _analyze_options_with_zerodha_data(self, symbol: str, market_data: Dict,
        ...
        """Analyze options using Zerodha data and enhanced Greeks with optimized parallel processing"""

        current_price = market_data['current_price']

        # Pre-calculate common values (avoid recalculation in loop)
        ...
        # Tag options with their type before processing
        all_options = [
        ...
        # Process options in parallel using ThreadPoolExecutor
        def analyze_single_option(option_data: Dict) -> Optional[Dict]:
            try:
                strike = option_data['strike']
                option_type = option_data['_option_type']
                iv = option_data.get('impliedVolatility', volatility / 100)

                # Calculate enhanced Greeks
                ...
                return {
                    'type': option_type,
                    'strike': strike,
                    'tradingsymbol': option_data.get('tradingsymbol', f"{symbol}28OCT{int(strike)}{'CE' if option_type == 'call' else 'PE'}"),
    ...
    def _generate_zerodha_strategy_with_technical(self, market_data: Dict, analyzed_options: List[Dict],
        ...
        # **[TARGET] ENHANCED STRATEGY SELECTION LOGIC**
    ...
    def _select_intraday_strategy(self, market_view: str, iv_percentile: float,
        ...
        # **[TARGET] ENHANCED INTRADAY STRATEGY SELECTION**
    ...
    def _select_swing_strategy(self, market_view: str, iv_percentile: float, risk_tolerance: str) -> Dict:
        ...
        # **[TARGET] ENHANCED SWING STRATEGY SELECTION**
        
        # **STRONG BULLISH CONVICTION**
        if market_view == 'strongly_bullish':
            if iv_percentile < 30:
                # Low IV = cheap options = buy outright
                strategy = 'LONG_CALL'  # [OK] FIXED: Was 'BULLISH_CALL'
        ...
        # **STRONG BEARISH CONVICTION**
            ...
            if iv_percentile < 30:
                # Low IV = cheap options = buy outright
                strategy = 'LONG_PUT'  # [OK] FIXED: Was 'BEARISH_PUT'
    ...
    def _create_zerodha_option_legs(self, strategy: Dict, analyzed_options: List[Dict],
        ...
        try:
            ...
            # [OK] FIXED: Added all the new strategy names
            if strategy_name in [
                'LONG_CALL', 'BULLISH_CALL',  # [OK] Added LONG_CALL
                'INTRADAY_LONG_CALL', 'INTRADAY_LONG_ATM_CALL'  # [OK] Added INTRADAY_LONG_CALL
            ]:
                legs = self._create_single_call_legs(liquid_options, risk_per_trade, spot)
            
            elif strategy_name in [
                'LONG_PUT', 'BEARISH_PUT',  # [OK] Added LONG_PUT
                'INTRADAY_LONG_PUT', 'INTRADAY_LONG_ATM_PUT'  # [OK] Added INTRADAY_LONG_PUT
            ]:
            ...
            # === DEFAULT FALLBACK ===
                ...
                print(f"[WARNING] Unknown strategy '{strategy_name}', using fallback")
            ...
            print(f"[ERROR] Error creating {strategy_name}: {e}")
        ...
        # **[TARGET] ENHANCED DEBUGGING OUTPUT**
        if legs:
            print(f"[OK] Created {len(legs)} legs for {strategy_name}")
            total_cost = 0
            total_margin = 0
            ...
            print(f"   [MONEY] Total Cost: ₹{total_cost:,.0f} | Estimated Margin: ₹{total_margin:,.0f}")
            
            # **Strategy validation**
            strategy_validation = self._validate_strategy_structure(legs, strategy_name)
            if strategy_validation['valid']:
                print(f"   [OK] Strategy structure validated: {strategy_validation['description']}")
            else:
                print(f"   [WARNING] Strategy structure warning: {strategy_validation['warning']}")
        else:
            print(f"[ERROR] No legs created for {strategy_name}")
    ...
    def _create_bear_put_spread_legs(self, liquid_options: List[Dict], risk_per_trade: float, spot: float) -> List[OptionsLeg]:
        ...
        if atm_puts and otm_puts:
            ...
            if min_contracts > 0:
                ...
                print(f"   [OK] Created bear put spread: {min_contracts} contracts each")
                return [buy_leg, sell_leg]
            else:
                print(f"   [ERROR] Zero contracts calculated, trying fallback")
        ...
        if len(all_puts) >= 2:
            ...
            if sell_put:
                ...
                if min_contracts > 0:
                    ...
                    print(f"   [OK] Created fallback bear put spread: {min_contracts} contracts each")
                    return [buy_leg, sell_leg]
        ...
        if all_puts:
            ...
            if buy_leg.contracts > 0:
                print(f"   [OK] Created single put: BUY {buy_leg.contracts} ₹{best_put['strike']} PUT")
                return [buy_leg]
        
        print(f"   [ERROR] No suitable puts found for bear put spread")
        return []
    ...
    def _create_bull_put_spread_legs(self, liquid_options: List[Dict], risk_per_trade: float, spot: float) -> List[OptionsLeg]:
        ...
        if not atm_puts or not otm_puts:
            print("[ERROR] Bull Put Spread: Insufficient put options found")
            return []
        ...
        if sell_put['strike'] <= buy_put['strike']:
            ...
            if lower_otm_puts:
                ...
                print("[ERROR] Bull Put Spread: Cannot find properly spaced strikes")
                return []
        
        print(f"[UP] Bull Put Spread: Sell ₹{sell_put['strike']} Put, Buy ₹{buy_put['strike']} Put")
        ...
        print(f"   [MONEY] Net Credit: ₹{net_credit:.0f}, Max Profit: ₹{max_profit:.0f}")
    ...
    def _create_bear_call_spread_legs(self, liquid_options: List[Dict], risk_per_trade: float, spot: float) -> List[OptionsLeg]:
        ...
        if not atm_calls or not otm_calls:
            print("[ERROR] Bear Call Spread: Insufficient call options found")
            return []
        ...
        if sell_call['strike'] >= buy_call['strike']:
            ...
            if higher_otm_calls:
                ...
                print("[ERROR] Bear Call Spread: Cannot find properly spaced strikes")
                return []
        
        print(f"[DOWN] Bear Call Spread: Sell ₹{sell_call['strike']} Call, Buy ₹{buy_call['strike']} Call")
    ...
    def _create_iron_butterfly_legs(self, liquid_options: List[Dict], risk_per_trade: float, spot: float) -> List[OptionsLeg]:
        ...
        if not all_strikes:
            print("[ERROR] No strikes found for Iron Butterfly")
            return []
        ...
        # [OK] CORRECTED Iron Butterfly structure:
        ...
        for action, opt_type, target_strike, description in butterfly_legs:
            ...
            if best_option:
                ...
                if fallback_options:
                    ...
                    print(f"   [WARNING] {description}: Using fallback strike ₹{fallback_option['strike']}")
                else:
        ...
        if len(temp_legs) != 4:
            print(f"   [ERROR] Iron Butterfly incomplete: Only {len(temp_legs)}/4 legs available")
            return []
        ...
        print(f"   [MONEY] Estimated Net Credit: ₹{net_credit:.2f} per lot")
        print(f"   [TARGET] Max Loss per lot: ₹{max_loss_per_lot:.2f}")
        print(f"   [CHART] Contracts calculated: {contracts} (Risk: ₹{risk_per_trade:.0f})")
        ...
        for action, opt_type, option_data, description in temp_legs:
            ...
            print(f"   [OK] {description}: {action} {contracts}x @ ₹{option_data['strike']} "
        ...
        if len(legs) == 4:
            ...
            if len(sell_legs) == 2 and len(buy_legs) == 2:
                ...
                if sell_types == ['call', 'put'] and buy_types == ['call', 'put']:
                    print("   [OK] Iron Butterfly structure validated: Correct option types")
                    ...
                    print(f"   [MONEY] Final Net Credit: ₹{net_credit:.0f}")
                    print(f"   [DOWN] Max Loss: ₹{max_loss:.0f}")
                    print(f"   [UP] Max Profit (at ATM): ₹{net_credit:.0f}")
                    print(f"   [TARGET] Risk/Reward Ratio: {abs(max_loss/net_credit):.2f}:1" if net_credit > 0 else "")
                    ...
                    if len(put_strikes) == 2 and len(call_strikes) == 2:
                        if put_strikes[1] == call_strikes[0]:  # ATM strikes should match
                            print(f"   [OK] Strike alignment verified: ATM at ₹{put_strikes[1]}")
                        else:
                            print(f"   [WARNING] ATM strikes misaligned: Put ATM ₹{put_strikes[1]}, Call ATM ₹{call_strikes[0]}")
                else:
                    print(f"   [WARNING] Incorrect option types: Sells {sell_types}, Buys {buy_types}")
            else:
                print("   [WARNING] Iron Butterfly structure warning: Incorrect BUY/SELL ratio")
    ...
    def _create_butterfly_spread_legs(self, liquid_options: List[Dict], risk_per_trade: float, spot: float, strategy_name: str) -> List[OptionsLeg]:
        ...
        if not atm_options:
            print(f"[ERROR] {opt_type.title()} Butterfly: No ATM {opt_type}s found")
            return []
        ...
        for tolerance_info in tolerance_levels:
            ...
            print(f"\n[TARGET] Trying {tolerance_desc} tolerance (±₹{tolerance:.0f})...")
            ...
            for action, option_type, target_strike, ratio, description in butterfly_strikes:
                ...
                if suitable_options:
                    ...
                    print(f"   [OK] {description}: {action} {leg.contracts}x @ ₹{best_option['strike']} "
                        f"(target: ₹{target_strike}, deviation: {deviation_percent:.1f}%)")
                else:
                    print(f"   [ERROR] {description}: No {option_type} within ±₹{tolerance:.0f} of ₹{target_strike}")
            
            # CHECK IF BUTTERFLY IS COMPLETE
            if len(legs) == 3:
                print(f"\n[OK] Butterfly complete with {tolerance_desc} tolerance!")
                ...
                if center_contracts != wing_contracts * 2:
                    print(f"   [WARNING] Adjusting contracts: Wings {wing_contracts}, Center {center_contracts} → {wing_contracts * 2}")
                ...
                print(f"   [CHART] Butterfly Quality Score: {quality_score:.1f}/100")
                print(f"   📏 Contract Ratio: {wing_contracts}-{wing_contracts * 2}-{wing_contracts} (1-2-1) [OK]")
                ...
                print(f"   [MONEY] Net Cost: ₹{estimated_cost:.0f}")
                print(f"   [MONEY] Max Profit (est): ₹{max_profit_theoretical:.0f}")
                ...
                if quality_score >= 70 or priority == 'low':  # Accept good quality or if we're at maximum tolerance
                    if quality_score >= 85:
                        ...
                        print(f"   [OK] Good quality butterfly created with {tolerance_desc} tolerance")
                    else:
                        print(f"   [WARNING] Acceptable butterfly created with {tolerance_desc} tolerance")
                    
                    return legs
                else:
                    print(f"   [WARNING] Quality score {quality_score:.1f} below threshold, trying next tolerance level...")
                    # Continue to next tolerance level
                    continue
            
            elif len(legs) == 2:
                print(f"   [WARNING] Partial butterfly ({len(legs)}/3 legs) with {tolerance_desc} tolerance")
            else:
                print(f"   [ERROR] Insufficient legs ({len(legs)}/3) with {tolerance_desc} tolerance")
        ...
        print(f"   [ERROR] All fallback strategies failed")
        return []
        ...
        for center_option in available_options[:3]:  # Try top 3 ATM options
            ...
            if lower_options and upper_options:
                ...
                for action, option, ratio, desc in [
                    ...
                    legs.append(leg)
                    print(f"      [OK] {desc}: {action} {leg.contracts}x @ ₹{option['strike']}")
                
                if len(legs) == 3:
                    print(f"   [OK] Modified butterfly created with strikes: ₹{lower_option['strike']}-₹{center_strike}-₹{upper_option['strike']}")
                    return legs
    ...
    def _create_butterfly_to_spread_fallback(self, liquid_options: List[Dict], opt_type: str, 
        ...
        if atm_options and otm_options:
            ...
            print(f"      [OK] Spread: BUY ₹{atm_option['strike']} / SELL ₹{otm_option['strike']}")
            return [buy_leg, sell_leg]
    ...
    def _create_iron_condor_legs(self, liquid_options: List[Dict], risk_per_trade: float, spot: float) -> List[OptionsLeg]:
        ...
        for action, opt_type, target_strike, description in condor_legs:
            ...
            if suitable_options:
                ...
                legs.append(leg)
                print(f"   [OK] {description}: {action} {leg.contracts} {best_option['tradingsymbol']}")
    ...
    def _validate_option_legs(self, legs: List[OptionsLeg], strategy_name: str) -> List[OptionsLeg]:
        ...
        if not legs:
            print(f"[WARNING] No legs created for {strategy_name}")
            return legs
        ...
        for i, leg in enumerate(legs):
            ...
            if issues:
                print(f"   [WARNING] Leg {i+1} issues: {', '.join(issues)}")
            else:
                print(f"   [OK] Leg {i+1}: {leg.action} {leg.contracts}x{leg.lot_size} {leg.tradingsymbol}")
        ...
        if strategy_key:
            ...
            if len(valid_legs) != expected.get('count', 0):
                print(f"   [WARNING] Expected {expected.get('count')} legs, got {len(valid_legs)}")
            ...
            if 'actions' in expected and len(valid_legs) == len(expected['actions']):
                ...
                if actual_actions != expected['actions']:
                    print(f"   [WARNING] Expected actions {expected['actions']}, got {actual_actions}")
            ...
            if 'ratios' in expected and len(valid_legs) == len(expected['ratios']):
                ...
                expected_contracts = [base_contracts * ratio for ratio in expected['ratios']]
                if actual_contracts != expected_contracts:
                    print(f"   [WARNING] Adjusting butterfly ratios from {actual_contracts} to {expected_contracts}")
            ...
            if expected.get('same_strike') and len(valid_legs) >= 2:
                ...
                if len(set(strikes)) > 1:
                    print(f"   [WARNING] {strategy_name} should use same strike, got {strikes}")
            
            # Check different strikes requirement  
            if expected.get('different_strikes') and len(valid_legs) >= 2:
                strikes = [leg.strike for leg in valid_legs]
                if len(set(strikes)) == 1:
                    print(f"   [WARNING] {strategy_name} should use different strikes, all at {strikes[0]}")
        ...
        if is_unlimited_risk and net_premium > 0:
            ...
            print(f"   [WARNING] Consider position sizing and stop losses!")
    ...
    def _validate_option_legs(self, legs: List[OptionsLeg], strategy_name: str) -> List[OptionsLeg]:
        ...
        for leg in legs:
            if leg.theoretical_price < 0.5:  # Less than ₹0.50
                print(f"[WARNING] Low premium warning: {leg.tradingsymbol} @ ₹{leg.theoretical_price:.2f}")
                # Reduce contracts for very low premium options
                leg.contracts = max(1, leg.contracts // 2)
        ...
        for strategy_key, expected_count in expected_legs.items():
            if strategy_key in strategy_name:
                if len(legs) != expected_count:
                    print(f"[WARNING] {strategy_name} validation: Expected {expected_count} legs, got {len(legs)}")
                break
    ...
    def _create_zerodha_leg(self, action: str, option_data: Dict, risk_amount: float) -> OptionsLeg:
        ...
        if action == 'BUY':
            ...
            print(f"      [CHART] Margin Calc for SELL {tradingsymbol}:")
...
async def main():
    ...
    print("[ROCKET] Zerodha-Enhanced Options Analyzer v4.0")
    print("=" * 80)
    ...
    try:
        ...
        for i, test in enumerate(test_cases):
            print(f"\n[CHART] Test Case {i+1}: Analyzing {test['symbol']} for {test['style']} trading...")
            ...
            if not result.get('error'):
                print(f"[OK] Analysis complete for {test['symbol']}")
                
                # Zerodha integration status
                zerodha_integration = result.get('zerodha_integration', {})
                print(f"   [SIGNAL] Data Source: {zerodha_integration.get('market_data_source', 'unknown')}")
                ...
                if trade_rec:
                    print(f"   [UP] Strategy: {trade_rec.get('strategy', 'Unknown')}")
                    print(f"   [TARGET] Confidence: {trade_rec.get('confidence', 0):.1%}")
                    ...
                    if zerodha_exec:
                        ...
                        print(f"   [MONEY] Estimated Margin: ₹{zerodha_exec.get('estimated_margin', 0):,.0f}")
                        print(f"   [OK] Execution Ready: {'Yes' if zerodha_exec.get('execution_ready') else 'No'}")
                
                # Risk management
                risk_mgmt = result.get('risk_management', {})
                if risk_mgmt and not risk_mgmt.get('error'):
                    print(f"   [WARNING]  Risk Score: {risk_mgmt.get('overall_risk_score', 0):.0f}/100")
                    print(f"   [OK] Risk Approved: {'Yes' if risk_mgmt.get('approved') else 'No'}")
                
                # Portfolio impact
                portfolio = result.get('portfolio_impact', {})
                if portfolio:
                    greeks = portfolio.get('greeks', {})
                    print(f"   [CHART] Portfolio Delta: {greeks.get('delta', 0):.3f}")
                ...
                if exec_results:
                    ...
                    print(f"   [ROCKET] Orders Executed: {len(successful_orders)}/{len(exec_results)}")
                    
                    for order in successful_orders:
                        print(f"      [OK] {order.get('order_id', 'N/A')}: {order.get('message', 'Order placed')}")
            else:
        ...
        if not position_status.get('error'):
            ...
            if net_positions:
                ...
                for pos in net_positions[:3]:  # Show first 3
                    print(f"   [UP] {pos['tradingsymbol']}: {pos['quantity']} @ ₹{pos['average_price']:.2f} (P&L: ₹{pos['pnl']:.0f})")
            else:
